...
{
	class AdaptiveThresholdSauvola
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
                ...
                CAdaptiveThresholdSauvola adaptiveThresholdSauvola = new CAdaptiveThresholdSauvola();

				// Source 이미지 설정 // Set source image 
				adaptiveThresholdSauvola.SetSourceImage(ref fliSrcImage);

				// Destination 이미지 설정 // Set destination image
				adaptiveThresholdSauvola.SetDestinationImage(ref fliDstImage);

                // 커널 사이즈 입력 // Set Kernel size 
                adaptiveThresholdSauvola.SetKernel(25, 25);

				// 민감도 설정 // Set Sensitivity 
				adaptiveThresholdSauvola.SetSensitivity(0.1);

				// 임계값 옵셋 설정 // Set threshold offset 
				adaptiveThresholdSauvola.SetThresholdOffset(new CMultiVar<double>(5));
