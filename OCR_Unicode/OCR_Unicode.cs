	...
	internal class OCR_Unicode
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				// 이미지 로드 // Load image
				if((res = fliImage.Load("../../ExampleImages/OCR/OCR_Learn_Unicode.flif")).IsFail())
				{
				...
				// 이미지 로드 // Load image
				if((res = fliRecognizeImage.Load("../../ExampleImages/OCR/OCR_Recognition_Unicode.flif")).IsFail())
				{
					ErrorPrint(res, "Failed to load the image file.\n");
					break;
				}
				...
				// 인식할 문자의 색상을 설정
				if((res = ocr.SetRecognizingCharacterColorType(ECharacterColorType.WhiteOnBlack)).IsFail())
				{
