...
namespace WeightedGuidedFilter
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
			...
			do
	        {
                ...
                if ((res = fliSourceImage.Load("../../ExampleImages/WeightedGuidedFilter/cat_450x480.flif")).IsFail())
                {
                ...
                if ((res = fliOperandImage.Load("../../ExampleImages/WeightedGuidedFilter/cat_450x480.flif")).IsFail())
                {
                    ErrorPrint(res, "Failed to load the image file. \n");
                    break;
                }
                ...
                if ((res = fliDestinationImage.Load("../../ExampleImages/WeightedGuidedFilter/cat_450x480.flif")).IsFail())
                {
                    ErrorPrint(res, "Failed to load the image file. \n");
                    break;
                }
				...
				CWeightedGuidedFilter weightedGuidedFilter = new CWeightedGuidedFilter();

				// Source 이미지 설정 // Set the source image
				weightedGuidedFilter.SetSourceImage(ref fliSourceImage);

				// Operand 이미지 설정 // Set the operand image
				weightedGuidedFilter.SetOperandImage(ref fliOperandImage);
