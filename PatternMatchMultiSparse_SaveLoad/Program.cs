...
namespace Match
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			CPatternMatchMultiSparse FLPatternMatchMultiSparseSave = new CPatternMatchMultiSparse();
			CPatternMatchMultiSparse FLPatternMatchMultiSparseLoad = new CPatternMatchMultiSparse();
			...
			do
			{
				...
				for(long i64DataIdx = 0; i64DataIdx < 2; ++i64DataIdx)
				{
					...
					FLPatternMatchMultiSparseSave.SetLearnImage(ref fliLearnImage[i64DataIdx]);
					...
					if((eResult = layerLearn.DrawFigureImage(arrLearnRegion[i64DataIdx], EColor.BLACK, 3)).IsFail())
						break;

					if((eResult = layerLearn.DrawFigureImage(arrLearnRegion[i64DataIdx], arrColor[i64DataIdx])).IsFail())
						break;
					...
					if((eResult = layerLearn.DrawTextCanvas(flpPosition00, strStatus, EColor.YELLOW, EColor.BLACK, 30)).IsFail())
						break;
				...
				if((eResult = FLPatternMatchMultiSparseSave.Save("../../ExampleImages/Matching/Pattern Multi Learn")).IsFail())
				{
				...
				// 이미지 로드 // Load image
				if((eResult = fliFindImage.Load("../../ExampleImages/Matching/Pattern Multi Find.flif")).IsFail())
					break;

				// 이미지 뷰 생성 // Create image view
				if((eResult = viewImageFind.Create(400, 384, 1168, 960)).IsFail())
					break;
				...
				for(long i64DataIdx = 0; i64DataIdx < 2; ++i64DataIdx)
				{
					...
					if((eResult = viewImageFind.SynchronizeWindow(ref viewImageLearn[i64DataIdx])).IsFail())
						break;
				}
				...
				if((eResult = FLPatternMatchMultiSparseLoad.Load("../../ExampleImages/Matching/Pattern Multi Learn")).IsFail())
				{
				...
				FLPatternMatchMultiSparseLoad.SetSourceImage(ref fliFindImage);
				...
				FLPatternMatchMultiSparseLoad.SetAngleTolerance(10.0);
				...
				// 알고리즘 수행 // Execute the Algoritm
				if((eResult = FLPatternMatchMultiSparseLoad.Execute()).IsFail())
				{
				...
				long i64ResultCount = FLPatternMatchMultiSparseLoad.GetResultCount();

				for(long i = 0; i < i64ResultCount; ++i)
				{
					CPatternMatchMultiSparse.SResult results;

					FLPatternMatchMultiSparseLoad.GetResult(i, out results);
					...
					if((eResult = layerFind.DrawFigureImage(flrResultRegion, arrColor[i64Idx])).IsFail())
						break;
					...
					if((eResult = layerFind.DrawTextImage(tpPosition, wstrClassName, EColor.YELLOW, EColor.BLACK, 30, false, 0, EGUIViewImageTextAlignment.CENTER)).IsFail())
						break;
