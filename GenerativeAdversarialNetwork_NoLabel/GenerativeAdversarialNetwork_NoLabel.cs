...
{
	class GenerativeAdversarialNetwork_NoLabel
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				if((res = viewImageResult.Create(1124, 0, 1636, 512)).IsFail())
				{
					ErrorPrint(res, "Failed to create the image view. \n");
					break;
				}
				...
				if((res = layerResult.DrawTextCanvas(flpPoint, "INFERENCE", EColor.YELLOW, EColor.BLACK, 30)).IsFail())
				{
					ErrorPrint(res, "Failed to draw text\n");
					break;
				}
				...
				generativeAdversarialNetworkDL.SetInferenceResultImage(ref fliResultImage);
				
				// 학습할 Generative Adversarial Network 모델 설정 // Set up the Generative Adversarial Network model to learn
				generativeAdversarialNetworkDL.SetModel(CGenerativeAdversarialNetworkDL.EModel.FLGenNet);
				// 학습할 Generative Adversarial Network 모델 설정 // Set up the Generative Adversarial Network model to learn
				generativeAdversarialNetworkDL.SetModelVersion(CGenerativeAdversarialNetworkDL.EModelVersion.FLGenNet_V1_64);
				// 학습 epoch 값을 설정 // Set the learn epoch value 
				generativeAdversarialNetworkDL.SetLearningEpoch(300);
				...
				generativeAdversarialNetworkDL.SetInferenceResultCount(100);
