	...
	class LinearInference
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				CResult res;
				// 이미지 로드 // Load image
				if((res = (arrFliImage[0].Load("../../ExampleImages/LinearInference/LinearInference_Src.flif"))).IsFail())
				{
				...
				// 이미지 로드 // Load image
				if((res = (arrFliImage[1].Load("../../ExampleImages/LinearInference/LinearInference_Opr.flif"))).IsFail())
				{
					ErrorPrint(res, "Failed to load the image file.\n");
					break;
				}
				...
				CLinearInference linearInference = new CLinearInference();
				// Source 이미지 설정 // Set source image
				linearInference.SetSourceImage(ref arrFliImage[0]);
				// Operand 이미지 설정 // Set Operand image
				linearInference.SetOperandImage(ref arrFliImage[1]);
				// Destination 이미지 설정 // Set destination image 
				linearInference.SetDestinationImage(ref arrFliImage[2]);
