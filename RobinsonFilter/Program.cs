    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            do
            {
				...
				// 두 이미지 뷰의 시점을 동기화 한다 // Synchronize the viewpoints of the two image views
				if((result = arrViewImage[0].SynchronizePointOfView(arrViewImage[2])).IsFail())
				{
					ErrorPrint(result, "Failed to synchronize view\n");
					break;
				}
				...
				// 두 이미지 뷰 윈도우의 위치를 맞춤 // Synchronize the positions of the two image view windows
				if((result = arrViewImage[0].SynchronizeWindow(arrViewImage[2])).IsFail())
				{
					ErrorPrint(result, "Failed to synchronize window.\n");
					break;
				}
				...
				CRobinsonFilter robinsonFilter = new CRobinsonFilter();
                // Source 이미지 설정 // Set source image
                robinsonFilter.SetSourceImage(ref arrFliImage[0]);
                // Destination 이미지 설정 // Set destination image
                robinsonFilter.SetDestinationImage(ref arrFliImage[1]);
                ...
                {
                    ErrorPrint(result, "Failed to execute RobinsonFilter.");
                    Console.WriteLine(result.GetString());
                    break;
                }

				// Destination 이미지 설정 // Set destination image
				robinsonFilter.SetDestinationImage(ref arrFliImage[2]);
                // 연산 모드 설정 // Set calculating mode
				...
				if((result = arrViewImage[2].SetFloatingImageValueRange(-180, 180)).IsFail())
				{
					ErrorPrint(result, "Failed to set value range of the image view.");
					Console.WriteLine(result.GetString());
					break;
				}
                ...
                if ((result = arrLayer[1].DrawTextCanvas(tpPosition, "Destination Image(AMP)", EColor.YELLOW, EColor.BLACK, 30)).IsFail())
                {
                    ErrorPrint(result, "Failed to draw text\n");
                    break;
                }

				if((result = arrLayer[2].DrawTextCanvas(tpPosition, "Destination Image(DIR)", EColor.YELLOW, EColor.BLACK, 30)).IsFail())
				{
					ErrorPrint(result, "Failed to draw text\n");
					break;
				}
