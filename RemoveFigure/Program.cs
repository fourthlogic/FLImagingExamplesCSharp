	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				strExpression1 = string.Format("vertexcount > 1 or area > 400 || (center.x > 300 || center.y < 100)");
				...
				if((eResult = (flfaResult1.RemoveFigure(strExpression1))).IsFail())
				{
				...
				// 조건식을 만족하는 Figure를 제거 // Remove the figure that satisfies the conditional expression
				if((eResult = (flfaResult2.RemoveFigure(strExpression2))).IsFail())
				{
					ErrorPrint(eResult, "Failed to process.\n");
					break;
				}
