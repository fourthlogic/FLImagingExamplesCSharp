...
{
	class OperationHypotenuse_Scalar
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				CResult res;
				// 이미지 로드 // Load image
				if((res = (arrFliImage[0].Load("../../ExampleImages/OperationHypotenuse/Sky.flif"))).IsFail())
				{
				...
				if((res = (arrLayer[0].DrawTextCanvas(tpPosition, "Source Image", EColor.YELLOW, EColor.BLACK, 18))).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}

				if((res = (arrLayer[1].DrawTextCanvas(tpPosition, "Destination1 Image(Hypotenuse 100, 100, 100)", EColor.YELLOW, EColor.BLACK, 18))).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}

				if((res = (arrLayer[2].DrawTextCanvas(tpPosition, "Destination2 Image(Hypotenuse 0, 0, 0)", EColor.YELLOW, EColor.BLACK, 18))).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}
