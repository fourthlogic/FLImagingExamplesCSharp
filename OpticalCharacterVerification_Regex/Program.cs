	...
	internal class Program
	{
		...
		static void Main(string[] args)
		{
			do
			{
				...
				// 이미지 로드 // Load image
				if((res = fliImage1.Load("../../ExampleImages/OpticalCharacterVerification/(주)포스로직.flif")).IsFail())
				{
					ErrorPrint(res, "Failed to load the image file.\n");
					break;
				}

				if((res = fliImage2.Load("../../ExampleImages/OpticalCharacterVerification/(주)포스로직2.flif")).IsFail())
				{
					ErrorPrint(res, "Failed to load the image file.\n");
					break;
				}
				...
				// OCR Font 파일을 로드
				if((res = ocv.LoadFontData("../../ExampleImages/OpticalCharacterRecognition/OCR_FourthLogic.flocr")).IsFail())
				{
