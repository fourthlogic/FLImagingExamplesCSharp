	...
	class IterativeClosestPoint3D
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				if((eResult = floLearnObject.Load("../../ExampleImages/CoordinateFrameUnification3D/Office_mosaicked(Left).ply")).IsFail())
				{
				...
				if((eResult = floSourceObject.Load("../../ExampleImages/CoordinateFrameUnification3D/Office_mosaicked(Middle).ply")).IsFail())
				{
					ErrorPrint(eResult, "Failed to load the object file.\n");
					break;
				}
				...
				// IterativeClosestPoint3D 객체 생성 // Create IterativeClosestPoint3D object
				CIterativeClosestPoint3D IterativeClosestPoint3D = new CIterativeClosestPoint3D();

				// Learn object 설정 // Set the learn object
				IterativeClosestPoint3D.SetLearnObject(ref floLearnObject);

				// Source object 설정 // Set the source object
				IterativeClosestPoint3D.SetSourceObject(ref floSourceObject);
				...
				if((eResult = IterativeClosestPoint3D.Execute()).IsFail())
				{
					ErrorPrint(eResult, "Failed to execute Iterative Closest Point 3D.");
					break;
				}
				...
				IterativeClosestPoint3D.GetMergedResult3DObject(true, true, tp3LearnColor, tp3SourceColor, ref floMergedResult);
