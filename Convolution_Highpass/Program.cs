...
namespace Convolution_Highpass
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			{
				// 이미지 로드 // Load image
				if((eResult = arrFliImage[(int)EType.Source].Load("../../ExampleImages/Convolution/Binding.flif")).IsFail())
				{
				...
				CConvolutionHighpass convolutionHighpass = new CConvolutionHighpass();
				// Source 이미지 설정 // Set the source image
				convolutionHighpass.SetSourceImage(ref arrFliImage[(int)EType.Source]);
				// Source ROI 설정 // Set the Source ROI
				convolutionHighpass.SetSourceROI(flrROI);

				// Destination1 이미지 설정 // Set the destination1 image
				convolutionHighpass.SetDestinationImage(ref arrFliImage[(int)EType.Destination1]);
				...
				convolutionHighpass.SetKernelMethod(CConvolutionHighpass.EKernel.Highpass1);

				// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
				if((eResult = (convolutionHighpass.Execute())).IsFail())
				{
				...
				convolutionHighpass.SetDestinationImage(ref arrFliImage[(int)EType.Destination2]);
				// Destination2 ROI 설정 // Set the destination2 ROI
				convolutionHighpass.SetDestinationROI(flrROI);

				// 커널 연산 방법 설정 // Set kernel operation method
				convolutionHighpass.SetKernelMethod(CConvolutionHighpass.EKernel.Highpass2);
				...
				if((eResult = (arrLayer[(int)EType.Destination1].DrawTextCanvas(flpZero, "Destination1 Image (Highpass1)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = (arrLayer[(int)EType.Destination2].DrawTextCanvas(flpZero, "Destination2 Image (Highpass2)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}
