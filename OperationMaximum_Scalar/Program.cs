	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				CResult eResult;
				// 이미지 로드 // Load image
				if((eResult = (arrFliImage[0].Load("../../ExampleImages/OperationMaximum/Flower.flif"))).IsFail())
				{
				...
				if((eResult = (arrLayer[1].DrawTextCanvas(tpPosition, "Destination1 Image(Maximum 100)", EColor.YELLOW, EColor.BLACK, 25))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = (arrLayer[2].DrawTextCanvas(tpPosition, "Destination2 Image(Maximum 200)", EColor.YELLOW, EColor.BLACK, 25))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}
