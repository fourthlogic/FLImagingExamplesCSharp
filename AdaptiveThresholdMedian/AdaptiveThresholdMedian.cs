	...
	class AdaptiveThresholdMedian
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				adaptiveThresholdMedian.SetSourceImage(ref fliSrcImage);

				// Destination 이미지 설정 // Set destination image
				adaptiveThresholdMedian.SetDestinationImage(ref fliDstImage);

                // 커널 사이즈 입력 // Set Kernel size 
                adaptiveThresholdMedian.SetKernel(25, 25);

				// 임계값 옵셋 설정 // Set threshold offset 
                adaptiveThresholdMedian.SetThresholdOffset(new CMultiVar<double>(5));
