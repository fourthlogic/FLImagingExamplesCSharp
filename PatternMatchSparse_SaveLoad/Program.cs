...
namespace Match
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			{
				// 이미지 로드 // Load image
				if((eResult = fliLearnImage.Load("../../ExampleImages/Matching/Pattern2 Single Learn.flif")).IsFail())
					break;

				if((eResult = fliFindImage.Load("../../ExampleImages/Matching/Pattern2 Single Find.flif")).IsFail())
					break;
				...
				if((eResult = FLPatternMatchSparseSave.Learn()).IsFail())
					break;

				// 데이터 Save를 진행합니다. // Proceed to save data.
				if((eResult = FLPatternMatchSparseSave.Save("../../ExampleImages/Matching/Pattern Single Learn")).IsFail())
				{
				...
				if((eResult = FLPatternMatchSparseLoad.Load("../../ExampleImages/Matching/Pattern Single Learn")).IsFail())
				{
				...
				// 알고리즘 수행 // Execute the Algoritm
				if((eResult = FLPatternMatchSparseLoad.Execute()).IsFail())
				{
				...
				for(long i = 0; i < i64ResultCount; ++i)
				{
					...
					FLPatternMatchSparseLoad.GetResult(i, out results);
