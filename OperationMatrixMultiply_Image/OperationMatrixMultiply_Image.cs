...
{
	class OperationMatrixMultiply_Image
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				if((res = arrFliImage[(int)EType.Operand].Load("../../ExampleImages/OperationMatrixMultiply/Matrix1.flif")).IsFail())
				{
					ErrorPrint(res, "Failed to load the image file.\n");
					break;
				}
				...
				operationMatrixMultiply.SetOperandImage(ref arrFliImage[(int)EType.Operand]);
				// Destination 이미지 설정 // Set the destination image
				operationMatrixMultiply.SetDestinationImage(ref arrFliImage[(int)EType.Destination]);

				// 연산 방식 설정 // Set operation source
				operationMatrixMultiply.SetOperationSource(EOperationSource.Image);

				// Overflow 처리 방식 설정 // Set Overflow Method
