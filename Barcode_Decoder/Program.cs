	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				// 이미지 로드 // Load image
				if((eResult = fliImage.Load("../../ExampleImages/Barcode/Barcode.flif")).IsFail())
				{
				...
				if((eResult = sBarcode.Execute()).IsFail())				
				...
				for(Int32 i = 0; i < i64Results; i++)
				{
					...
					if((eResult = sBarcode.GetResultDataRegion(i, out flqRegion)).IsFail())
					{
					...
					if((eResult = sBarcode.GetResultDecodedString(i, out strDecodedMsg)).IsFail())
					{
					...
					if((eResult = layer.DrawTextImage(flqRegion.flpPoints[0], strSymbol, EColor.YELLOW, EColor.BLACK, 12, false, 0, EGUIViewImageTextAlignment.LEFT_BOTTOM)).IsFail())
					{
						ErrorPrint(eResult, "Failed to draw string object on the image view.");
						continue;
					}

					if((eResult = layer.DrawTextImage(flqRegion.flpPoints[3], strDecodedMsg, EColor.CYAN, EColor.BLACK, 20)).IsFail())
					{
						ErrorPrint(eResult, "Failed to draw string object on the image view.");
						continue;
					}
