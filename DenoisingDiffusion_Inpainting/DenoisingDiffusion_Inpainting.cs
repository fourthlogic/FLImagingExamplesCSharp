...
{
	class DenoisingDiffusion_Inpainting
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				// 이미지 로드 // Load image
				if((res = fliLearnImage.Load("../../ExampleImages/DenoisingDiffusion/Ball_Train.flif")).IsFail())
				{
					ErrorPrint(res, "Failed to load the image file. \n");
					break;
				}

				if((res = fliValidateIamge.Load("../../ExampleImages/DenoisingDiffusion/Ball_Validation.flif")).IsFail())
				{
					ErrorPrint(res, "Failed to load the image file. \n");
					break;
				}

				if((res = fliSourceImage.Load("../../ExampleImages/DenoisingDiffusion/Ball_Source.flif")).IsFail())
				{
					ErrorPrint(res, "Failed to load the image file. \n");
					break;
				}
				...
				CDenoisingDiffusionInpaintingDL denoisingDiffusionInpaintingDL = new CDenoisingDiffusionInpaintingDL();

				// OptimizerSpec 객체 생성 // Create OptimizerSpec object
				COptimizerSpecAdamGradientDescent optSpec = new COptimizerSpecAdamGradientDescent();

				// 학습할 이미지 설정 // Set the image to learn
				denoisingDiffusionInpaintingDL.SetLearningImage(ref fliLearnImage);
				// 검증할 이미지 설정 // Set the image to validate
				denoisingDiffusionInpaintingDL.SetLearningValidationImage(ref fliValidateIamge);
				...
				denoisingDiffusionInpaintingDL.SetInterpolationMethod(EInterpolationMethod.Bilinear);
				...
				denoisingDiffusionInpaintingDL.EnableOptimalLearningStatePreservation(true);
				...
				denoisingDiffusionInpaintingDL.EnableLearningGradientClipping(true);
				denoisingDiffusionInpaintingDL.SetLearningGradientClippingThreshold(1.0f);
				// 설정한 Optimizer를 GAN에 적용 // Apply the Optimizer that we set up to GAN
				denoisingDiffusionInpaintingDL.SetLearningOptimizerSpec(optSpec);
				...
				autoSaveSpec.SetAutoSavePath("model.flddi");
				...
				{
					eLearnResult = denoisingDiffusionInpaintingDL.Learn();
					bTerminated = true;
				}, null);
				...
				while(!denoisingDiffusionInpaintingDL.IsRunning() && !bTerminated)
					Thread.Sleep(1);

				int i32MaxEpoch = denoisingDiffusionInpaintingDL.GetLearningEpoch();
				int i32PrevEpoch = 0;
				...
				while(true)
				{
					...
					int i32MaxIteration = denoisingDiffusionInpaintingDL.GetActualMiniBatchCount();
					// 마지막 미니 배치 반복 횟수 받기 // Get the last number of mini batch iterations
					int i32Iteration = denoisingDiffusionInpaintingDL.GetLearningResultCurrentIteration();
					// 마지막 학습 횟수 받기 // Get the last epoch learning
					int i32Epoch = denoisingDiffusionInpaintingDL.GetLastEpoch();
					...
					if(i32Epoch != i32PrevEpoch && i32Iteration == i32MaxIteration && i32Epoch > 0)
					{
						...
						denoisingDiffusionInpaintingDL.GetLearningResultAllHistory(ref vctCosts, ref vctSSIM, ref vctMRQ, ref vctValidationEpoch);
						...
						if(vctCosts.Count != 0)
						{
							...
							float f32MRQ = vctMRQ.Count != 0 ? vctMRQ.Last() : 0;

							// 해당 epoch의 비용과 검증 결과 값 출력 // Print cost and validation value for the relevant epoch
							Console.WriteLine("Cost : {0:F6} SSIM : {1:F6} MRQ : {2:F6} Epoch {3} / {4}", f32CurrCost, f32SSIM, f32MRQ, i32Epoch, i32MaxEpoch);
							...
							if((vctCosts.Count() != 0 && i32PrevCostCount != vctCosts.Count()) || (vctSSIM.Count() != 0 && i32PrevSSIMCount != vctSSIM.Count()))
							{
								...
								int i32Step = denoisingDiffusionInpaintingDL.GetLearningValidationStep();
								...
								for(long i = 0; i < vctMRQ.Count() - 1; ++i)
									flaX.Add((float)(i * i32Step));

								flaX.Add((float)(vctCosts.Count() - 1));
								// Graph View 데이터 입력 // Input Graph View Data
								viewGraph.Plot(flaX, vctMRQ, EChartType.Line, EColor.BLUE, "MRQ");
							...
							if(bEscape)
								denoisingDiffusionInpaintingDL.Stop();
					...
					if(!denoisingDiffusionInpaintingDL.IsRunning())
						break;
				}
				...
				denoisingDiffusionInpaintingDL.SetInferenceImage(ref fliSourceImage);

				// 생성할 이미지 설정 // Set the image to create
				denoisingDiffusionInpaintingDL.SetInferenceResultImage(ref fliResultImage);

				// 알고리즘 수행 // Execute the algorithm
				if((res = denoisingDiffusionInpaintingDL.Execute()).IsFail())
				{
