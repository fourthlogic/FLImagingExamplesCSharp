	...
	class DepthImageNearestNeighborInterpolation3D
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				CResult res;
				// 이미지 로드 // Load image
				if((res = fliSourceImage.Load("../../ExampleImages/DepthImageNearestNeighborInterpolation3D/Source.flif")).IsFail())
				{
				...
				depthImageNearestNeighborInterpolation3D.SetStartPosition(0);

				// End Position 설정 // Set end position

				depthImageNearestNeighborInterpolation3D.SetEndPosition(2);
