	...
	class MeasurementDL
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				measurementDL.SetLearningImage(ref fliLearnImage);
				// 검증할 이미지 설정 // Set the image to validate
				measurementDL.SetLearningValidationImage(ref fliValidationImage);
				// 분류할 이미지 설정 // Set the image to classify
				measurementDL.SetInferenceImage(ref fliValidationImage);
				...
				while(!measurementDL.IsRunning() && !bTerminated)
					Thread.Sleep(1);

				int i32MaxEpoch = measurementDL.GetLearningEpoch();
				int i32PrevEpoch = 0;
				...
				while(true)
				{
					...
					int i32MiniBatchCount = measurementDL.GetActualMiniBatchCount();
					// 마지막 미니 배치 반복 횟수 받기 // Get the last number of mini batch iterations
					int i32Iteration = measurementDL.GetLearningResultCurrentIteration();
					// 마지막 학습 횟수 받기 // Get the last epoch learning
					int i32Epoch = measurementDL.GetLastEpoch();
					...
					if(i32Epoch != i32PrevEpoch && i32Iteration == i32MiniBatchCount && i32Epoch > 0)
					{
						...
						float f32CurrCost = measurementDL.GetLearningResultLastCost();
						// 마지막 검증 결과 받기 // Get the last validation result
						float f32ValidationPa = measurementDL.GetLearningResultLastAccuracy();
						float f32ValidationPaMeanIoU = measurementDL.GetLearningResultLastMeanIoU();
						...
						{
							int i32Step = measurementDL.GetLearningValidationStep();
