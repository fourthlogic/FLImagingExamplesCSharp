	...
	class ChannelLinfNorm
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				CResult res;
				// 이미지 로드 // Load image
				if((res = fliSrcImage.Load("../../ExampleImages/ChannelLinfNorm/Coord.flif")).IsFail())
				{
				...
				CChannelLinfNorm channelLinfNorm = new CChannelLinfNorm();

				if((res = channelLinfNorm.SetSourceImage(ref fliSrcImage)).IsFail())
					break;
				if((res = channelLinfNorm.SetDestinationImage(ref fliDstImage)).IsFail())
					break;

				// 알고리즘 수행 // Execute the algorithm
				if((res = channelLinfNorm.Execute()).IsFail())
				{
