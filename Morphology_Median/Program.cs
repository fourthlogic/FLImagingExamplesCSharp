	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				// Blob 객체 생성
				CMorphologyMedian morphologyMedian = new CMorphologyMedian();

				// 처리할 이미지 설정
				morphologyMedian.SetSourceImage(ref fliSrcImage);

				// ROI 범위 설정
				CFLRect<Int32> flrROI = new CFLRect<int>(100, 190, 360, 420);

				// Src ROI 설정
				morphologyMedian.SetSourceROI(flrROI);

				// Dst 이미지 설정
				morphologyMedian.SetDestinationImage(ref fliDstImage);

				// Dst ROI 설정
				morphologyMedian.SetDestinationROI(flrROI);

				// 처리할 Morphology의 Kernel Size 설정 (KernelSize = 5 일 경우, Kernel Size : 5x5, 홀수만 설정가능)
				morphologyMedian.SetKernelSize(5);

				// 앞서 설정된 파라미터 대로 알고리즘 수행
				if((ipResult = morphologyMedian.Execute()).IsFail())
				{
					Console.WriteLine("Failed to execute morphology median.");
