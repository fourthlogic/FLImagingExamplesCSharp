...
{
	class AdaptiveThresholdWolfJolion
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
                ...
                // Adaptive Threshold Wolf Jolion 객체 생성 // Create Adaptive Threshold Wolf Jolion object
                CAdaptiveThresholdWolfJolion adaptiveThresholdWolfJolion = new CAdaptiveThresholdWolfJolion();

				// Source 이미지 설정 // Set source image 
				adaptiveThresholdWolfJolion.SetSourceImage(ref fliSrcImage);

				// Destination 이미지 설정 // Set destination image
				adaptiveThresholdWolfJolion.SetDestinationImage(ref fliDstImage);

                // 커널 사이즈 입력 // Set Kernel size 
                adaptiveThresholdWolfJolion.SetKernel(25, 25);

				// 민감도 설정 // Set Sensitivity 
				adaptiveThresholdWolfJolion.SetSensitivity(0.1);

				// 임계값 옵셋 설정 // Set threshold offset 
				adaptiveThresholdWolfJolion.SetThresholdOffset(new CMultiVar<double>(5));

				// 알고리즘 수행 // Execute the algorithm
				if((res = (adaptiveThresholdWolfJolion.Execute())).IsFail())
				{
					ErrorPrint(res, "Failed to execute Adaptive Threshold Wolf Jolion.");
					break;
				}
