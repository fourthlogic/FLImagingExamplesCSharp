...
namespace Morphology_TopHat
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			{
				// 이미지 로드 // Load image
				if((eResult = arrFliImage[(int)EType.Source].Load("../../ExampleImages/Morphology/CityNight.flif")).IsFail())
				{
				...
				// 이미지 뷰 생성 // Create image view
				if((eResult = (arrViewImage[(int)EType.Region].Create(400, 400, 600, 600))).IsFail())
				{
					ErrorPrint(eResult, "Failed to create the image view.\n");
					bError = true;
					break;
				...
				// 이미지 뷰에 이미지를 디스플레이 // Display an image in an image view
				if((eResult = (arrViewImage[(int)EType.Region].SetImagePtr(ref arrFliImage[(int)EType.Region]))).IsFail())
				{
					ErrorPrint(eResult, "Failed to set image object on the image view.\n");
					bError = true;
					break;
				...
				CMorphologyTopHat morphologyTopHat = new CMorphologyTopHat();
				// Source 이미지 설정 // Set the source image
				morphologyTopHat.SetSourceImage(ref arrFliImage[(int)EType.Source]);
				// Source ROI 설정 // Set the Source ROI
				morphologyTopHat.SetSourceROI(flrROI);

				// Destination 이미지 설정 // Set the destination image
				morphologyTopHat.SetDestinationImage(ref arrFliImage[(int)EType.Destination]);
				// Destination ROI 설정 // Set Destination ROI
				morphologyTopHat.SetDestinationROI(flrROI);
				// 연산 방법 설정
				morphologyTopHat.SetTransformMethod(CMorphologyTopHat.ETransformMethod.White);

				// 처리할 Morphology Kernel 의 (95, 75, 100, 80, 45.000000) 설정 // Set Morphology Kernel to L(95, 75, 100, 80, 45.000000)
				CFLRect<int> flrRegion = new CFLRect<int>(95, 75, 100, 80, 45.000000);
				morphologyTopHat.SetKernel(flrRegion);

				// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
				if((eResult = (morphologyTopHat.Execute())).IsFail())
				{
				...
				arrLayer[(int)EType.Region] = arrViewImage[(int)EType.Region].GetLayer(0);
				arrLayer[(int)EType.Region].Clear();

				if((eResult = (arrLayer[(int)EType.Region].DrawFigureImage(flrRegion, EColor.LIME))).IsFail())
					ErrorPrint(eResult, "Failed to draw figure.\n");
				...
				if((eResult = (arrLayer[(int)EType.Region].DrawTextCanvas(flpZero, "Region Shape", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}
