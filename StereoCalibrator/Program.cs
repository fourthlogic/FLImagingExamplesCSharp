...
namespace StereoCalibrator
{
	class Program
	{
		...
		public struct SGridDisplay
		{
			...
			public CStereoCalibrator.SGridResult sGridData;
		};
		...
		static bool Calibration(CStereoCalibrator sSC, CFLImage fliLearnImage, CFLImage fliLearnImage2)
		{
			bool bResult = false;
		...
		static bool Undistortion(CStereoCalibrator sSC, CFLImage fliSourceImage, CFLImage fliSourceImage2, CFLImage fliDestinationImage, CFLImage fliDestinationImage2)
		{
			bool bResult = false;
		...
		static void Main(string[] args)
		{
			...
			CStereoCalibrator sSC = new CStereoCalibrator();
			...
			do
			{
				...
				{
					sSC.GetResultGridPoints(out sArrGridDisplay[i64ImgIdx].sGridData, i64ImgIdx);
					sArrGridDisplay[i64ImgIdx].i64ImageIdx = i64ImgIdx;
				}

				for(long i64ImgIdx = 0; i64ImgIdx < (long)fliLearnImage2.GetPageCount(); ++i64ImgIdx)
				{
					sSC.GetResultGridPoints2(out sArrGridDisplay2[i64ImgIdx].sGridData, i64ImgIdx);
