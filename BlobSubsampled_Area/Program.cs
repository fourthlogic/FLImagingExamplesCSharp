	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				Int32 i32ResultTypeMask = (Int32)CBlob.EBlobResultType.Contour | (Int32)CBlob.EBlobResultType.Area;
				...
				if((eResult = sBlob.Filter(CBlob.EFilterItem.Area, 100, ELogicalCondition.Less)).IsFail())
				{
					ErrorPrint(eResult, "BlobSubsampled filtering algorithm error occurred.");
					break;
				}

				// BlobSubsampled 결과를 얻어오기 위해 FigureArray, List<double> 선언
				...
				if((eResult = sBlob.GetResultAreas(out flaArea)).IsFail())
				{
