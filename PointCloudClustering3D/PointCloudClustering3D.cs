	...
	class PointCloudClustering3D
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				if((eResult = floSourceObject.Load("../../ExampleImages/Smoothing3D/Smoothing3DExample.ply")).IsFail())
				{
				...
				CPointCloudClustering3D pointCloudClustering3D = new CPointCloudClustering3D();

				// Source object 설정 // Set the source object
				pointCloudClustering3D.SetSourceObject(ref floSourceObject);
				...
				{
					ErrorPrint(eResult, "Failed to execute Point Cloud Clustering 3D.");
					break;
				}
				...
				if((eResult = layer3DDst.DrawTextCanvas(flp, "Clustering Result", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}
				...
				int i32ResultCount = pointCloudClustering3D.GetResultClusterCount();

				for(int i = 0; i < i32ResultCount; ++i)
				{
					CFL3DObject flo = new CFL3DObject();
					pointCloudClustering3D.GetResult3DObject(i, ref flo);

					TPoint3<Byte> tp3Color = new TPoint3<Byte>();
					CColorPaletteUtilities.GetColor(i + 1, ref tp3Color.x, ref tp3Color.y, ref tp3Color.z);

					int i32VertexCount = (int)flo.GetVertices().Count;
					List<TPoint3<Byte>> listColors = new List<TPoint3<byte>>();
					listColors.Capacity = i32VertexCount;

					for(int idx = 0; idx < i32VertexCount; ++idx)
						listColors.Add(tp3Color);

					flo.SetVertexColors(listColors);
					...
					flo.GetBoundingBox(ref tp3Min, ref tp3Max);

					tp3Center.x = (double)((tp3Min.x + tp3Max.x) / 2);
					tp3Center.y = (double)((tp3Min.y + tp3Max.y) / 2);
					tp3Center.z = (double)((tp3Min.z + tp3Max.z) / 2);

					layer3DDst.DrawText3D(tp3Center, String.Format("{0}", i), EColor.YELLOW, EColor.BLACK, 10);
				}
