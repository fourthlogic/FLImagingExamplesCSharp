...
namespace BilinearRegionInterpolation
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			{
				// 이미지 로드 // Load image
				if((eResult = arrFliImage[(int)EType.Source].Load("../../ExampleImages/RegionInterpolation/Sky_Damaged.flif")).IsFail())
				{
				...
				if((eResult = (sBRI.Execute())).IsFail())
				{
				...
				if((eResult = (arrLayer[(int)EType.Destination].DrawTextCanvas(flpZero, "Destination Image (Bilinear)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}
