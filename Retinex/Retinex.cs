	...
	class Retinex
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				CResult res;
				// 이미지 로드 // Load image
				if((res = (arrFliImage[0].Load("../../ExampleImages/Retinex/wall.flif"))).IsFail())
				{
				...
				// Retinex 객체 생성 // Create Retinex object
				CRetinex retinex = new CRetinex();
				// Source 이미지 설정 // Set source image
				retinex.SetSourceImage(ref arrFliImage[0]);
				// Destination 이미지 설정 // Set destination image 
				retinex.SetDestinationImage(ref arrFliImage[1]);

				// 정규화 범위 설정 //Set Normalization Range
				CMultiVar<double> mvMinRange = new CMultiVar<double>(0.000000);
				CMultiVar<double> mvMaxRange = new CMultiVar<double>(255.000000);

				retinex.SetMinNormalizationRange(mvMinRange);
				retinex.SetMaxNormalizationRange(mvMaxRange);

				//Intensity Clamping Ratio 설정 // Set Intensity Clamping Ratio
				CMultiVar<double> mvMinRatio = new CMultiVar<double>(0.010000);
				CMultiVar<double> mvMaxRatio = new CMultiVar<double>(0.010000);

				retinex.SetMinIntensityClampingRatio(mvMinRatio);
				retinex.SetMaxIntensityClampingRatio(mvMaxRatio);

				// 알고리즘 수행 // Execute the algorithm
				if((res = (retinex.Execute())).IsFail())
				{
					ErrorPrint(res, "Failed to execute Retinex.");
					break;
				}
