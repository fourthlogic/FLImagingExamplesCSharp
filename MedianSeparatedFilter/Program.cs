	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				// 이미지 로드 // Load image
				if ((eResult =fliSrcImage.Load("../../ExampleImages/MedianFilter/Chip_Noise.flif")).IsFail())
				{
				...
				if ((eResult =(eResult = filterMedianSeparate.Execute())).IsFail())
				{
					ErrorPrint(eResult, "Failed to execute MedianSeparate Filter.");
					break;
				}
				...
				if ((eResult =layerDst.DrawFigureImage(flrROI, EColor.BLUE)).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw figures objects on the image view.\n");
					break;
				}
