...
namespace FLImagingExamplesCSharp
{
	class DenoisingDiffusion
	{
		...
		static void Main(string[] args)
		{
			...
			CFLImage fliResultImage0 = new CFLImage();
			...
			CGUIViewImage viewImageResult0 = new CGUIViewImage();
			...
			do
			{
				...
				// 이미지 로드 // Load image
				if((res = fliLearnImage.Load("../../ExampleImages/GenerativeAdversarialNetwork/mnist1000.flif")).IsFail())
				{
					ErrorPrint(res, "Failed to load the image file. \n");
					break;
				}

				if((res = fliValidateIamge.Load("../../ExampleImages/GenerativeAdversarialNetwork/mnist100.flif")).IsFail())
				{
					ErrorPrint(res, "Failed to load the image file. \n");
					break;
				}
				...
				if((res = viewImageResult0.Create(100, 512, 612, 1024)).IsFail())
				{
					ErrorPrint(res, "Failed to create the image view. \n");
					break;
				}

				if((res = viewImageResult1.Create(612, 512, 1124, 1024)).IsFail())
				{
					ErrorPrint(res, "Failed to create the image view. \n");
					break;
				}

				if((res = viewImageResult2.Create(1124, 512, 1636, 1024)).IsFail())
				{
					ErrorPrint(res, "Failed to create the image view. \n");
					break;
				}
				...
				if((res = viewGraph.Create(1124, 0, 1636, 512)).IsFail())
				{
				...
				if((res = viewImageResult0.SetImagePtr(ref fliResultImage0)).IsFail())
				{
					ErrorPrint(res, "Failed to set image object on the image view. \n");
					break;
				}
				...
				CGUIViewImageLayer layerResult0 = viewImageResult0.GetLayer(0);
				...
				if((res = layerResult0.DrawTextCanvas(flpPoint, "0", EColor.YELLOW, EColor.BLACK, 30)).IsFail())
				{
					ErrorPrint(res, "Failed to draw text\n");
					break;
				}

				if((res = layerResult1.DrawTextCanvas(flpPoint, "1", EColor.YELLOW, EColor.BLACK, 30)).IsFail())
				{
					ErrorPrint(res, "Failed to draw text\n");
					break;
				}

				if((res = layerResult2.DrawTextCanvas(flpPoint, "2", EColor.YELLOW, EColor.BLACK, 30)).IsFail())
				{
					ErrorPrint(res, "Failed to draw text\n");
					break;
				}

				// Denoising Diffusion 객체 생성 // Create Denoising Diffusion object
				CDenoisingDiffusionDL denoisingDiffusionDL = new CDenoisingDiffusionDL();

				// OptimizerSpec 객체 생성 // Create OptimizerSpec object
				COptimizerSpecAdamGradientDescentDL optSpec = new COptimizerSpecAdamGradientDescentDL();

				// 학습할 이미지 설정 // Set the image to learn
				denoisingDiffusionDL.SetLearningImage(ref fliLearnImage);
				// 검증할 이미지 설정 // Set the image to validate
				denoisingDiffusionDL.SetLearningValidationImage(ref fliValidateIamge);
				...
				denoisingDiffusionDL.EnableLearningGradientClipping(true);
				...
				autoSaveSpec.SetAutoSavePath("model.fldd");
				...
				// 자동 저장 옵션 설정 // Set Auto-Save Options
				denoisingDiffusionDL.SetLearningAutoSaveSpec(autoSaveSpec);

				// DenoisingDiffusion learn function을 진행하는 스레드 생성 // Create the DenoisingDiffusion Learn function thread
				CResult eLearnResult = new CResult();

				ThreadPool.QueueUserWorkItem((arg) =>
				{
					eLearnResult = denoisingDiffusionDL.Learn();
					bTerminated = true;
				}, null);
				...
				while(!denoisingDiffusionDL.IsRunning() && !bTerminated)
					Thread.Sleep(1);

				int i32MaxEpoch = denoisingDiffusionDL.GetLearningEpoch();
				int i32PrevEpoch = 0;
				...
				while(true)
				{
					...
					int i32MaxIteration = denoisingDiffusionDL.GetActualMiniBatchCount();
					// 마지막 미니 배치 반복 횟수 받기 // Get the last number of mini batch iterations
					int i32Iteration = denoisingDiffusionDL.GetLearningResultCurrentIteration();
					// 마지막 학습 횟수 받기 // Get the last epoch learning
					int i32Epoch = denoisingDiffusionDL.GetLastEpoch();
					...
					if(i32Epoch != i32PrevEpoch && i32Iteration == i32MaxIteration && i32Epoch > 0)
					{
						...
						denoisingDiffusionDL.GetLearningResultAllHistory(ref vctCosts, ref vctSSIM, ref vctPDV, ref vctValidationEpoch);
						...
						if(vctCosts.Count != 0)
						{
							...
							if((vctCosts.Count() != 0 && i32PrevCostCount != vctCosts.Count()) || (vctSSIM.Count() != 0 && i32PrevSSIMCount != vctSSIM.Count()))
							{
								...
								int i32Step = denoisingDiffusionDL.GetLearningValidationStep();
					...
					if(!denoisingDiffusionDL.IsRunning())
						break;
				}
				...
				denoisingDiffusionDL.SetInferenceResultImage(ref fliResultImage0);
				...
				denoisingDiffusionDL.SetInferenceClassWeight(liClassWeight);

				// 알고리즘 수행 // Execute the algorithm
				if((res = denoisingDiffusionDL.Execute()).IsFail())
				{
				...
				// 생성할 이미지 설정 // Set the image to create
				denoisingDiffusionDL.SetInferenceResultImage(ref fliResultImage1);
				...
				// 생성할 이미지 설정 // Set the image to create
				denoisingDiffusionDL.SetInferenceResultImage(ref fliResultImage2);
				...
				viewImageResult0.Invalidate(true);
				viewImageResult1.Invalidate(true);
				viewImageResult2.Invalidate(true);
				// 그래프 뷰를 갱신 // Update the Graph view.
				viewGraph.Invalidate(true);

				// 이미지 뷰가 종료될 때 까지 기다림 // Wait for the image view to close
				while(viewImageLearn.IsAvailable() && viewImageResult0.IsAvailable() && viewImageResult1.IsAvailable() && viewImageResult2.IsAvailable() && viewImageValidate.IsAvailable() && viewGraph.IsAvailable())
                    Thread.Sleep(1);

                viewImageLearn.Destroy();
                viewImageResult0.Destroy();
                viewImageResult1.Destroy();
                viewImageResult2.Destroy();
                viewImageValidate.Destroy();
                viewGraph.Destroy();
			}
			while(false);
		}
