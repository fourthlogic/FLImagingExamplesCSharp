	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				CMorphologyClose morphologyClose = new CMorphologyClose();
				// Source 이미지 설정 // Set the source image
				morphologyClose.SetSourceImage(ref arrFliImage[(int)EType.Source]);
				// Source ROI 설정 // Set the Source ROI
				morphologyClose.SetSourceROI(flrROI);

				// Destination 이미지 설정 // Set the destination image
				morphologyClose.SetDestinationImage(ref arrFliImage[(int)EType.Destination]);
                ...
                morphologyClose.SetKernel(flrRegion);

				// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
				if((eResult = (morphologyClose.Execute())).IsFail())
				{
