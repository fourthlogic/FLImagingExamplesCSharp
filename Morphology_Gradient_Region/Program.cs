	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				CMorphologyGradient morphologyGradient = new CMorphologyGradient();
				// Source 이미지 설정 // Set the source image
				morphologyGradient.SetSourceImage(ref arrFliImage[(int)EType.Source]);
				// Source ROI 설정 // Set the Source ROI
				morphologyGradient.SetSourceROI(flrROI);

				// Destination 이미지 설정 // Set the destination image
				morphologyGradient.SetDestinationImage(ref arrFliImage[(int)EType.Destination]);
				...
				morphologyGradient.SetKernel(flrRegion);

				// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
				if((eResult = (morphologyGradient.Execute())).IsFail())
				{
