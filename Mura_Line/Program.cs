	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				// 이미지 로드 // Load image
				if((eResult = fliImageSrc.Load("../../ExampleImages/Mura/Line.flif")).IsFail())
				{
				...
				if((eResult = viewImageSrc.SetImagePtr(ref fliImageSrc)).IsFail())
				{
				...
				if((eResult = sMura.Execute()).IsFail())
				{
				...
				if((eResult = sMura.Filter(CMura.EFilterItem.MinimumEnclosingRectangleLongSideLength, 50, ELogicalCondition.LessEqual)).IsFail())
				{
				...
				if((eResult = sMura.GetResultContours(out flfaContours)).IsFail())
				{
