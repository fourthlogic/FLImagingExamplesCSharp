    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
			...
			do
	        {
                ...
                if ((eResult = fliSourceImage.Load("../../ExampleImages/GuidedFilter/cat_450x480.flif")).IsFail())
                {
                ...
                if ((eResult = fliOperandImage.Load("../../ExampleImages/GuidedFilter/cat_450x480.flif")).IsFail())
                {
                    ErrorPrint(eResult, "Failed to load the image file. \n");
                    break;
                }
                ...
                if ((eResult = fliDestinationImage.Load("../../ExampleImages/GuidedFilter/cat_450x480.flif")).IsFail())
                {
                    ErrorPrint(eResult, "Failed to load the image file. \n");
                    break;
                }
				...
				CFastGuidedFilter fastGuidedFilter = new CFastGuidedFilter();

				// Source 이미지 설정 // Set the source image
				fastGuidedFilter.SetSourceImage(ref fliSourceImage);

				// Operand 이미지 설정 // Set the operand image
				fastGuidedFilter.SetOperandImage(ref fliOperandImage);
