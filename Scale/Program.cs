...
namespace Scale
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			{
				// 이미지 로드 // Load image
				if((eResult = arrFliImage[(int)EType.Source].Load("../../ExampleImages/Affine/Sea.flif")).IsFail())
				{
				...
				scale.SetSourceImage(ref arrFliImage[(int)EType.Source]);
				// Destination 이미지 설정 // Set the destination image
				scale.SetDestinationImage(ref arrFliImage[(int)EType.Destination1]);
				...
				if((eResult = (scale.Execute())).IsFail())
				{
				...
				// Destination 이미지 설정 // Set the destination image
				scale.SetDestinationImage(ref arrFliImage[(int)EType.Destination2]);
				// Scale 변환 방식 비율로 설정 // Set to scale conversion method ratio
				scale.SetScaleMethod(EScaleMethod.Ratio);
				// Scale 비율 설정 // set scale ratio
				scale.SetScale(1.5, 1.5);
				...
				if((eResult = (arrLayer[(int)EType.Destination1].DrawTextCanvas(flpZero, "Destination1 Image (x1.5)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = (arrLayer[(int)EType.Destination2].DrawTextCanvas(flpZero, "Destination2 Image (x1.5 Resize)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = (arrLayer[(int)EType.Destination3].DrawTextCanvas(flpZero, "Destination3 Image (1024x1024 Resize)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}
