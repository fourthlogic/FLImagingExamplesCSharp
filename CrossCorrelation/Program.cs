	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				CResult eResult;
				// 이미지 로드 // Load image
				if((eResult = (arrFliImage[0].Load("../../ExampleImages/CrossCorrelation/Cross Correlation Source.flif"))).IsFail())
				{
				...
				// 이미지 로드 // Load image
				if((eResult = (arrFliImage[1].Load("../../ExampleImages/CrossCorrelation/Cross Correlation Operand.flif"))).IsFail())
				{
					ErrorPrint(eResult, "Failed to load the image file.\n");
					break;
				}
				...
				// Cross Correlation 객체 생성 // Create CrossCorrelation object
				CCrossCorrelation CrossCorrelation = new CCrossCorrelation();
				// Source 이미지 설정 // Set source image
				CrossCorrelation.SetSourceImage(ref arrFliImage[0]);
				// Operand 이미지 설정 // Set Operand image
				CrossCorrelation.SetOperandImage(ref arrFliImage[1]);
				// Destination 이미지 설정 // Set destination image 
				CrossCorrelation.SetDestinationImage(ref arrFliImage[2]);
