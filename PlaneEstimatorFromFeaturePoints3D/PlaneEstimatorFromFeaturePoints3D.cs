...
{
	class PlaneEstimatorFromFeaturePoints3D
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				if((eResult = floSourceObject.Load("../../ExampleImages/PlaneEstimatorFromFeaturePoints3D/box.fl3do")).IsFail())
				{
				...
				CPlaneEstimatorFromFeaturePoints3D planeEstimatorFromFeaturePoints3D = new CPlaneEstimatorFromFeaturePoints3D();

				// Destination object 설정 // Set the learn object
				planeEstimatorFromFeaturePoints3D.SetDestinationObject(ref floDestinationObject);
				// Source object 설정 // Set the source object
				planeEstimatorFromFeaturePoints3D.SetSourceObject(ref floSourceObject);

				// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
				if((eResult = planeEstimatorFromFeaturePoints3D.Execute()).IsFail())
				{
					ErrorPrint(eResult, "Failed to execute Plane Estimator From Feature Points 3D.");
					break;
				}
				...
				if((eResult = view3DDst.PushObject((CFL3DObject)planeEstimatorFromFeaturePoints3D.GetDestinationObject())).IsFail() ||
					(eResult = view3DDst.PushObject(floSourceObject)).IsFail())
				{
				...
				//view3DDst.Sync(ref view3DSource);
