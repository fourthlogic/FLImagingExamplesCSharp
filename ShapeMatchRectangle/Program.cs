...
namespace ShapeMatch
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			{
				// 이미지 로드 // Load image
				if((res = fliImage.Load("../../ExampleImages/ShapeMatch/Rect Match.flif")).IsFail())
				{
				...
				CShapeMatchRectangle shapeMatch = new CShapeMatchRectangle();
				...
				for(long i = 0; i < i64ResultCount; ++i)
				{
					...
					Console.Write("  1. Shape Type : Rectangle\n");
					...
					if((res = layer.DrawFigureImage(matchResult.flrResultObject, EColor.CYAN, 3)).IsFail())
					{
						ErrorPrint(res, "Failed to draw figure\n");
						break;
					}

					string str = String.Format("Score : {0}\nAngle : {0}\nScale : {1}", matchResult.f32Score, matchResult.f32Angle, matchResult.f32Scale);

					if((res = layer.DrawTextImage(matchResult.flrResultObject.GetCenter(), str, EColor.YELLOW, EColor.BLACK, 15)).IsFail())
					{
