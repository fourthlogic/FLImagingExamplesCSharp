	...
	class ChannelLpNorm
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				CChannelLpNorm channelLpNorm = new CChannelLpNorm();

				// Source 이미지 설정 // Set Source image
				if((res = channelLpNorm.SetSourceImage(ref fliSrcImage)).IsFail())
					break;

				// Operand 이미지 설정 // Set Operand image
				if((res = channelLpNorm.SetOperandImage(ref fliOprImage)).IsFail())
					break;

				// Destination 이미지 설정 // Set Destination image 
				if((res = channelLpNorm.SetDestinationImage(ref fliDstImage)).IsFail())
					break;
