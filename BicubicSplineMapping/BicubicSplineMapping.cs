	...
	class BicubicSplineMapping
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				for(int y = 0; y < bicubicSplineMapping.GetRow(); ++y)
				{
					for(int x = 0; x < bicubicSplineMapping.GetColumn(); ++x)
					{
						CBicubicSplineMapping.CBicubicSplineMappingVertexInfo vertex = bicubicSplineMapping.GetControlPoint(new CFLPoint<int>(x, y));
				...
				for(int y = 0; y <= (bicubicSplineMapping.GetRow() - 1) * f64Slice; ++y)
				{
					...
					for(int x = 0; x <= (bicubicSplineMapping.GetColumn() - 1) * f64Slice; ++x)
					{
						...
						if(bicubicSplineMapping.ConvertSourceToDestination(flpdSource, ref flpdDestination).IsOK())
						{
							...
							if(bicubicSplineMapping.ConvertDestinationToSource(flpdDestination, ref flpdConvertedSource).IsOK())
							{
