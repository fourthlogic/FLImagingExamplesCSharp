    ...
    class Statistics_Sum_SumOfSquare
	{
        ...
        static void Main(string[] args)
		{
			...
			do
			{
				...
				if((res = imageStatistics.GetSum(ref mvSum)).IsFail())
				{
                ...
                if ((res = imageStatistics.GetSumOfSquares(ref mvSumOfSquares)).IsFail()) // Function that calculate the sum of squares of the pixel value of the image(or the region of ROI)
				{
                    ErrorPrint(res, "Failed to process.");
                    break;
                }
                ...
                // 이미지 전체(혹은 ROI 영역) 픽셀값의 합을 구하는 함수 // Function that calculate the sum of the pixel value of the image(or the region of ROI)
                if((res = imageStatistics.GetSum(ref mvTrimmingSum)).IsFail())
				{
                    ErrorPrint(res, "Failed to process.");
                    break;
                }
                ...
                if((res = imageStatistics.GetSumOfSquares(ref mvTrimmingSumOfSquares)).IsFail())
				{
                    ErrorPrint(res, "Failed to process.");
                    break;
                }
                ...
                if((res = layer.DrawTextCanvas(flpPoint, strSumValue, EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
                ...
                // 이미지 뷰 정보 표시 // Display image view information
                if((res = layer.DrawTextCanvas(flpPoint, strSumOfSquaresValue, EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
                    ErrorPrint(res, "Failed to draw text.\n");
                    break;
                }
                ...
                // 이미지 뷰 정보 표시 // Display image view information
                if((res = layer.DrawTextCanvas(flpPoint, strTrimmingSumValue, EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
                    ErrorPrint(res, "Failed to draw text.\n");
                    break;
                }
                ...
                // 이미지 뷰 정보 표시 // Display image view information
                if((res = layer.DrawTextCanvas(flpPoint, strTrimmingSumOfSquaresValue, EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
                    ErrorPrint(res, "Failed to draw text.\n");
                    break;
                }
