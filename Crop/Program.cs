	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				CResult eResult;
				// 이미지 로드 // Load image
				if((eResult = fliISrcImage.Load("../../ExampleImages/Crop/bacteria.flif")).IsFail())
				{
				...
				// 이미지 뷰에 이미지를 디스플레이 // Display the image in the image view
				if((eResult = viewImage[1].SetImagePtr(ref fliICropImage)).IsFail())
				{
					ErrorPrint(eResult, "Failed to set image object on the image view.\n");
					break;
				}
				...
				if((eResult = layer2.DrawTextImage(flpTemp, "Crop Image", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
					ErrorPrint(eResult, "Failed to draw text.\n");

				// Crop 영역 표기 // Crop Area draw
				if((eResult = layer1.DrawFigureImage(flrCropROI, EColor.LIME)).IsFail())
