	...
	class SimplestColorBalance
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				// 이미지 로드 // Load image
				if((res = fliSrcImage.Load("../../ExampleImages/SimplestColorBalance/church.flif")).IsFail())
				{
				...
				// SimplestColorBalance 객체 생성 // Create SimplestColorBalance object
				CSimplestColorBalance simplestColorBalance = new CSimplestColorBalance();

				// 처리할 이미지 설정 // Set the image to process
				simplestColorBalance.SetSourceImage(ref fliSrcImage);

				// Destination 이미지 설정 // Set the destination image
				simplestColorBalance.SetDestinationImage(ref fliDst1Image);

				// 하위 Saturation 비율 설정 // Set the Lower Saturation Ratio
				simplestColorBalance.SetLowerSaturationRatio(0.01);

				// 상위 Saturation 비율 설정 // Set the Upper Saturation Ratio
				simplestColorBalance.SetUpperSaturationRatio(0.01);

				// Balance Mode 설정 // Set the balance mode
				simplestColorBalance.SetBalanceMode(CSimplestColorBalance.EBalanceMode.Color);

				// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
				if((res = simplestColorBalance.Execute()).IsFail())
				{
					ErrorPrint(res, "Failed to execute Simplest Color Balance.");
					break;
				}

				// Destination 이미지 설정 // Set the destination image
				simplestColorBalance.SetDestinationImage(ref fliDst2Image);

				// Balance Mode 설정 // Set the processing mode
				simplestColorBalance.SetBalanceMode(CSimplestColorBalance.EBalanceMode.Intensity);
				...
				// 아래 함수 DrawTextCanvas은 Screen 좌표를 기준으로 하는 String을 Drawing 한다. // The function DrawTextCanvas below draws a String based on the screen coordinates.
				...
				if((res = layerDst1.DrawTextCanvas(flp, ("Destination1 Image (Color)"), EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(res, "Failed to draw text\n");
					break;
				}

				if((res = layerDst2.DrawTextCanvas(flp, ("Destination2 Image (Intensity)"), EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(res, "Failed to draw text\n");
					break;
				}
