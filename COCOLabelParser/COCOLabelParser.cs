	...
	class COCOLabelParser
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				// 이미지 로드 // Load image
                if ((res = fliSourceImage.Load("../../ExampleImages/COCOLabelParser/Source/COCO_val2014_000000391895.jpg")).IsFail())
				{
				...
				if((res = layerResult1.DrawTextCanvas(flpPoint, "RESULT (Rect)", EColor.YELLOW, EColor.BLACK, 30)).IsFail())
				{
					ErrorPrint(res, "Failed to draw text\n");
					break;
				}

				if((res = layerResult2.DrawTextCanvas(flpPoint, "RESULT (Segmentation)", EColor.YELLOW, EColor.BLACK, 30)).IsFail())
				{
					ErrorPrint(res, "Failed to draw text\n");
					break;
				}
				...
				cocoLabelParserDL.SetDestinationImageFolderPath("../../ExampleImages/COCOLabelParser/Destination");
				...
				// 이미지 로드 // Load image
				if((res = fliResultImage1.Load("../../ExampleImages/COCOLabelParser/Destination/Result.flif")).IsFail())
				{
					ErrorPrint(res, "Failed to load the image file. \n");
					break;
				}
				...
				// 이미지 로드 // Load image
				if((res = fliResultImage2.Load("../../ExampleImages/COCOLabelParser/Destination/Result.flif")).IsFail())
				{
					ErrorPrint(res, "Failed to load the image file. \n");
					break;
				}
