	...
	class OperationComplexMultiply_Scalar
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				if((res = operationComplexMultiply.SetScalarValue(new CMultiVar<double>(2, 1))).IsFail())
				{
				...
				if((res = layerDestination.DrawTextCanvas(new CFLPoint<double>(0, 0), "Destination Image(Multiplied by 2 + 1i)", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}
