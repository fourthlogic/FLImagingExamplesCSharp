	...
	class ConvolutionFD_Image
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				CResult res;
				// 이미지 로드 // Load image
				if((res = (arrFliImage[0].Load("../../ExampleImages/ConvolutionFD/Sun.flif"))).IsFail())
				{
				...
				convolutionFD.SetSourceImage(ref arrFliImage[0]);
				// Destination 이미지 설정 // Set destination image 
				convolutionFD.SetDestinationImage(ref arrFliImage[1]);
				...
				filterGeneratorBandpassFD.SetMaxFrequency(0.5);
				filterGeneratorBandpassFD.SetFilterShape(CFilterGeneratorBandpassFD.EFilterShape.EFilterShape_Butterworth);
				filterGeneratorBandpassFD.SetDistance(50);
				filterGeneratorBandpassFD.SetDegree(7);

				CFilterGeneratorBaseFD filterGeneratorBaseFD = filterGeneratorBandpassFD;
				...
				// Destination 이미지 설정 // Set destination image 
				convolutionFD.SetDestinationImage(ref arrFliImage[2]);
				...
				if((res = (arrLayer[1].DrawTextCanvas(tpPosition, "Destination Image(Bandpass)", EColor.YELLOW, EColor.BLACK, 30))).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}

				if((res = (arrLayer[2].DrawTextCanvas(tpPosition, "Destination Image(Gaussian)", EColor.YELLOW, EColor.BLACK, 30))).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}
