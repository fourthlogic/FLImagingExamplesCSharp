...
namespace Gauge
{
    class CornerGauge_SpecificCorner
	{
		...
		static void Main(string[] args)
		{
			...
			{
				CCornerGauge.ECorner.LeftTop,
				CCornerGauge.ECorner.LeftBottom,
				CCornerGauge.ECorner.LeftTop_RightTop,
				CCornerGauge.ECorner.RightTop_RightBottom_LeftBottom,
			};
			...
			do
			{
				...
				for(int i = 0; i < i32ExampleCount; ++i)
				{
					...
					if((res = viewImage[i].Create(i32X, i32Y, i32X + 450, i32Y + 450)).IsFail())
					{
				...
				for(int i = 0; i < i32ExampleCount; ++i)
				{
					...
					if(res.IsOK())
					{
						...
						for(int j = 0; j < flfaResultCorner.GetCount(); ++j)
						{
							CFLPoint<double> flpCorner = (CFLPoint<double>)flfaResultCorner.GetAt(j);
							Console.WriteLine("Corner X : {0}, Corner Y : {1}", flpCorner.x, flpCorner.y);
						}
