...
namespace LaplacianOfGaussianFilter
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			{
				// 이미지 로드 // Load image
                if ((res = arrFliImage[(int)EType.Source].Load("../../ExampleImages/LaplacianOfGaussianFilter/Gear.flif")).IsFail())
				{
				...
				{
					ErrorPrint(res, "Failed to execute LaplacianOfGaussianFilter.");
					break;
				}
				...
				if((res = arrLayer[(int)EType.Destination2].DrawTextCanvas(flpZero, "Destination2 Image (Sigma 0.8)", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}

				if((res = arrLayer[(int)EType.Destination3].DrawTextCanvas(flpZero, "Destination3 Image (Sigma 1)", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}

				if((res = arrLayer[(int)EType.Destination4].DrawTextCanvas(flpZero, "Destination4 Image (Sigma 2)", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}
