...
namespace LaplacianOfGaussianFilter
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			{
				// 이미지 로드 // Load image
                if ((eResult = arrFliImage[(int)EType.Source].Load("../../ExampleImages/LaplacianOfGaussianFilter/Gear.flif")).IsFail())
				{
				...
				LaplacianOfGaussianFilter.SetSourceImage(ref arrFliImage[(int)EType.Source]);
				// Source ROI 설정 // Set the Source ROI
				LaplacianOfGaussianFilter.SetSourceROI(flrROI);

				// Destination1 이미지 설정 // Set the destination1 image
				LaplacianOfGaussianFilter.SetDestinationImage(ref arrFliImage[(int)EType.Destination1]);
				...
				{
					ErrorPrint(eResult, "Failed to execute LaplacianOfGaussianFilter.");
					break;
				}

				// Destination2 이미지 설정 // Set the destination2 image
				LaplacianOfGaussianFilter.SetDestinationImage(ref arrFliImage[(int)EType.Destination2]);
				...
				LaplacianOfGaussianFilter.SetDestinationImage(ref arrFliImage[(int)EType.Destination3]);
				...
				LaplacianOfGaussianFilter.SetDestinationImage(ref arrFliImage[(int)EType.Destination4]);
				...
				if((eResult = arrLayer[(int)EType.Destination1].DrawTextCanvas(flpZero, "Destination1 Image (Sigma 0.5)", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = arrLayer[(int)EType.Destination2].DrawTextCanvas(flpZero, "Destination2 Image (Sigma 0.8)", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = arrLayer[(int)EType.Destination3].DrawTextCanvas(flpZero, "Destination3 Image (Sigma 1)", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = arrLayer[(int)EType.Destination4].DrawTextCanvas(flpZero, "Destination4 Image (Sigma 2)", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}
