...
namespace Convolution_Laplacian
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			{
				// 이미지 로드 // Load image
				if((eResult = arrFliImage[(int)EType.Source].Load("../../ExampleImages/Convolution/Moss.flif")).IsFail())
				{
				...
				CConvolutionLaplacian convolutionLaplacian = new CConvolutionLaplacian();
				// Source 이미지 설정 // Set the source image
				convolutionLaplacian.SetSourceImage(ref arrFliImage[(int)EType.Source]);
				// Source ROI 설정 // Set the Source ROI
				convolutionLaplacian.SetSourceROI(flrROI);

				// Destination1 이미지 설정 // Set the destination1 image
				convolutionLaplacian.SetDestinationImage(ref arrFliImage[(int)EType.Destination1]);
				...
				convolutionLaplacian.SetKernelMethod(CConvolutionLaplacian.EKernel.LaplacianX);

				// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
				if((eResult = (convolutionLaplacian.Execute())).IsFail())
				{
				...
				convolutionLaplacian.SetDestinationImage(ref arrFliImage[(int)EType.Destination2]);
				// Destination2 ROI 설정 // Set the destination2 ROI
				convolutionLaplacian.SetDestinationROI(flrROI);

				// 커널 연산 방법 설정 // Set kernel operation method
				convolutionLaplacian.SetKernelMethod(CConvolutionLaplacian.EKernel.LaplacianY);
				...
				convolutionLaplacian.SetDestinationImage(ref arrFliImage[(int)EType.Destination3]);
				// Destination3 ROI 설정 // Set the destination3 ROI
				convolutionLaplacian.SetDestinationROI(flrROI);

				// 커널 연산 방법 설정 // Set kernel operation method
				convolutionLaplacian.SetKernelMethod(CConvolutionLaplacian.EKernel.Laplacian4);
				...
				convolutionLaplacian.SetDestinationImage(ref arrFliImage[(int)EType.Destination4]);
				// Destination4 ROI 설정 // Set the destination4 ROI
				convolutionLaplacian.SetDestinationROI(flrROI);

				// 커널 연산 방법 설정 // Set kernel operation method
				convolutionLaplacian.SetKernelMethod(CConvolutionLaplacian.EKernel.Laplacian8);
				...
				if((eResult = (arrLayer[(int)EType.Destination1].DrawTextCanvas(flpZero, "Destination1 Image (LaplacianX)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = (arrLayer[(int)EType.Destination2].DrawTextCanvas(flpZero, "Destination2 Image (LaplacianY)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = (arrLayer[(int)EType.Destination3].DrawTextCanvas(flpZero, "Destination3 Image (Laplacian4)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = (arrLayer[(int)EType.Destination4].DrawTextCanvas(flpZero, "Destination4 Image (Laplacian8)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}
