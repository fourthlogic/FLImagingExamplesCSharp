...
namespace Convolution_Laplacian
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			{
				// 이미지 로드 // Load image
				if((res = arrFliImage[(int)EType.Source].Load("../../ExampleImages/Convolution/Moss.flif")).IsFail())
				{
				...
				if((res = (convolutionLaplacian.Execute())).IsFail())
				{
				...
				if((res = (arrLayer[(int)EType.Destination1].DrawTextCanvas(flpZero, "Destination1 Image (LaplacianX)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}

				if((res = (arrLayer[(int)EType.Destination2].DrawTextCanvas(flpZero, "Destination2 Image (LaplacianY)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}

				if((res = (arrLayer[(int)EType.Destination3].DrawTextCanvas(flpZero, "Destination3 Image (Laplacian4)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}

				if((res = (arrLayer[(int)EType.Destination4].DrawTextCanvas(flpZero, "Destination4 Image (Laplacian8)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}
