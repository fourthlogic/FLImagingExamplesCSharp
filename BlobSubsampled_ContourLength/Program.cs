	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				// 면적이 50보다 작은 객체들을 제거
				if((eResult = sBlob.Filter(CBlob.EFilterItem.Area, 50, ELogicalCondition.Less)).IsFail())
				{
				...
				if((eResult = sBlob.Filter(CBlob.EFilterItem.Circularity, 0.7, ELogicalCondition.Less)).IsFail())
				{
					ErrorPrint(eResult, "BlobSubsampled filtering algorithm error occurred.");
					break;
				}
