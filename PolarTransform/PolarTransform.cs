	...
	class PolarTransform
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				CResult res;
				// 이미지 로드 // Load image
				if((res = (arrFliImage[0].Load("../../ExampleImages/PolarTransform/PolarTransformSrc.flif"))).IsFail())
				{
				...
				// PolarTransform 객체 생성 // Create PolarTransform object
				CPolarTransform polarTransform = new CPolarTransform();
				// Source 이미지 설정 // Set source image
				polarTransform.SetSourceImage(ref arrFliImage[0]);
				// Destination 이미지 설정 // Set destination image 
				polarTransform.SetDestinationImage(ref arrFliImage[1]);

				// 알고리즘 수행 // Execute the algorithm
				if((res = (polarTransform.Execute())).IsFail())
				{
					ErrorPrint(res, "Failed to execute Polar Transform.");
					break;
				}
