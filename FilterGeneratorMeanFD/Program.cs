	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				CResult eResult;
				// 이미지 로드 // Load image
				if((eResult = fliISrcImage.Load("../../ExampleImages/FilterGeneratorFD/Sea1Ch.flif")).IsFail())
				{
				...
				if((eResult = viewImage[2].Create(300 + 520 * 2, 0, 300 + 520 * 3, 430)).IsFail())
				{
					ErrorPrint(eResult, "Failed to create the image view.\n");
					break;
				}

				if((eResult = viewImage[3].Create(300, 0 + 430, 300 + 520, 430 * 2)).IsFail())
				{
					ErrorPrint(eResult, "Failed to create the image view.\n");
					break;
				}

				if((eResult = viewImage[4].Create(300 + 520, 0 + 430, 300 + 520 * 2, 430 * 2)).IsFail())
				{
					ErrorPrint(eResult, "Failed to create the image view.\n");
					break;
				}
				...
				if((eResult = viewImage[1].SetImagePtr(ref fliIFFTImage)).IsFail())
				{
					ErrorPrint(eResult, "Failed to set image object on the image view.\n");
					break;
				}

				if((eResult = viewImage[2].SetImagePtr(ref fliIFilterImage)).IsFail())
				{
					ErrorPrint(eResult, "Failed to set image object on the image view.\n");
					break;
				}

				if((eResult = viewImage[3].SetImagePtr(ref fliIMultiplyImage)).IsFail())
				{
					ErrorPrint(eResult, "Failed to set image object on the image view.\n");
					break;
				}

				if((eResult = viewImage[4].SetImagePtr(ref fliIDstImage)).IsFail())
				{
					ErrorPrint(eResult, "Failed to set image object on the image view.\n");
					break;
				}
				...
				CFilterGeneratorMeanFD filterGenerator = new CFilterGeneratorMeanFD();
				...
				filterGenerator.SetType(CFilterGeneratorMeanFD.EFilterBaseFDType.FFT_Shift);

				// Mask Shape 설정 // set Mask Shape
				filterGenerator.SetMaskShape(CFilterGeneratorMeanFD.EMaskShape.Rectangle);
				...
				if((eResult = layer1.DrawTextImage(flpTemp, "Source Image", EColor.YELLOW, EColor.BLACK, 30)).IsFail())
					ErrorPrint(eResult, "Failed to draw text.\n");

				if((eResult = layer2.DrawTextImage(flpTemp, "FFT Image", EColor.YELLOW, EColor.BLACK, 30)).IsFail())
					ErrorPrint(eResult, "Failed to draw text.\n");

				if((eResult = layer3.DrawTextImage(flpTemp, "Destination Image(Filter)", EColor.YELLOW, EColor.BLACK, 30)).IsFail())
					ErrorPrint(eResult, "Failed to draw text.\n");

				if((eResult = layer4.DrawTextImage(flpTemp, "Filtering FFT Image", EColor.YELLOW, EColor.BLACK, 30)).IsFail())
					ErrorPrint(eResult, "Failed to draw text.\n");

				if((eResult = layer5.DrawTextImage(flpTemp, "Filtering Image", EColor.YELLOW, EColor.BLACK, 30)).IsFail())
					ErrorPrint(eResult, "Failed to draw text.\n");
