	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				if((eResult = FLGeometricMatchMultiSave.Save("../../ExampleImages/Matching/Geometric Multi Learn")).IsFail())
				{
				...
				// 데이터를 Load합니다.
				if((eResult = FLGeometricMatchMultiLoad.Load("../../ExampleImages/Matching/Geometric Multi Learn")).IsFail())
				{
					ErrorPrint(eResult, "Failed to save\n");
					break;
				}
				...
				// 알고리즘 수행 // Execute the Algoritm
				if((eResult = FLGeometricMatchMultiLoad.Execute()).IsFail())
				{
