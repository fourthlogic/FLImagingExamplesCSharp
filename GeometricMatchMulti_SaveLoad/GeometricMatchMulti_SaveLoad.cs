...
{
	class GeometricMatchMulti_SaveLoad
	{
		...
		static void Main(string[] args)
		{
			...
			CGeometricMatchMulti geometricMatchMultiSave = new CGeometricMatchMulti();
			CGeometricMatchMulti geometricMatchMultiLoad = new CGeometricMatchMulti();
			...
			do
			{
				...
				for(long i64DataIdx = 0; i64DataIdx < 3; ++i64DataIdx)
				{
					...
					geometricMatchMultiSave.SetLearnImage(ref fliLearnImage[i64DataIdx]);
					...
					geometricMatchMultiSave.SetLearnROI(learnRegion);
					geometricMatchMultiSave.SetLearnPivot(flpLearnPivot);

					// 학습 파라미터를 설정합니다. // Set the learning parameters.
					// 추출할 특징점 개수를 설정합니다. // Set the number of feature points to be extracted.
					geometricMatchMultiSave.SetFeatureCount(2048);
					...
					geometricMatchMultiSave.SetFeatureFiltering(0.5);
					// 추출할 특징점 처리과정에서의 허용 임계값을 설정합니다. // Set the allowable threshold in the feature point processing process to be extracted.
					geometricMatchMultiSave.SetLearnThresholdCoefficient(1.3);

					// 알고리즘 수행 // Execute the Algoritm
					if((geometricMatchMultiSave.Learn(arrClassName[i64DataIdx])).IsFail())
						break;
					...
					geometricMatchMultiSave.GetLearnedFeature(ref flfaFeaturePoints);
				...
				if((res = geometricMatchMultiSave.Save("../../ExampleImages/Matching/Geometric Multi Learn")).IsFail())
				{
				...
				// 데이터를 Load합니다.
				if((res = geometricMatchMultiLoad.Load("../../ExampleImages/Matching/Geometric Multi Learn")).IsFail())
				{
					ErrorPrint(res, "Failed to save\n");
					break;
				}
				...
				geometricMatchMultiLoad.SetSourceImage(ref fliFindImage);
				...
				geometricMatchMultiLoad.SetAngleBias(0.0);
				...
				long i64ResultCount = geometricMatchMultiLoad.GetResultCount();
				...
				for(long i = 0; i < i64ResultCount; ++i)
				{
					...
					geometricMatchMultiLoad.GetResult(i, ref results);
					geometricMatchMultiLoad.GetResultDetectedFeature(i, ref flfaResultPoints);
