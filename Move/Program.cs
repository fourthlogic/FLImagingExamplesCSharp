...
namespace Move
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			{
				// 이미지 로드 // Load image
				if((eResult = arrFliImage[(int)EType.Source].Load("../../ExampleImages/Affine/Generator.flif")).IsFail())
				{
				...
				move.SetSourceImage(ref arrFliImage[(int)EType.Source]);
				...
				if((eResult = (move.Execute())).IsFail())
				{
				...
				for(int i = 0; i < (int)EType.ETypeCount; ++i)
				{
					...
					if((eResult = (arrLayer[i].DrawFigureImage(fleROI, EColor.LIME))).IsFail())
