	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				if((eResult = (lczm.Load("MappingData.fllcz"))).IsFail())
				{
					...
					if((eResult = (lczm.Save("MappingData.fllcz"))).IsFail())
					{
				...
				for(int y = 0; y <= (lczm.GetRow() - 1) * f64Slice; ++y)
				{
					...
					for(int x = 0; x <= (lczm.GetColumn() - 1) * f64Slice; ++x)
					{
						...
						if(lczm.ConvertSourceToTarget(flpdSource, out flpdTarget).IsOK())
						{
							...
							if(lczm.ConvertTargetToSource(flpdTarget, out flpdConvertedSource).IsOK())
							{
