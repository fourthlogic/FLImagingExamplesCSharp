	...
	class OperationMean_Scalar
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				CResult res;
				// 이미지 로드 // Load image
				if((res = (arrFliImage[0].Load("../../ExampleImages/OperationMean/Sky.flif"))).IsFail())
				{
				...
				operationMean.SetOperationSource(EOperationSource.Scalar);
				// Scalar 값 설정 // Set Scalar value
				operationMean.SetScalarValue(mvScalr2);
				...
				if((res = (arrLayer[1].DrawTextCanvas(tpPosition, "Destination1 Image(Mean 100, 100, 100)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}

				if((res = (arrLayer[2].DrawTextCanvas(tpPosition, "Destination2 Image(Mean 0, 0, 0)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}
