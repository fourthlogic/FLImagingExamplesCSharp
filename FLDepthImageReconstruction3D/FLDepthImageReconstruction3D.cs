...
{
	class FLDepthImageReconstruction3D
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				CFLDepthImageReconstruction3D flDepthImageReconstruction3D = new CFLDepthImageReconstruction3D();

				if((res = flDepthImageReconstruction3D.SetSourceImage(ref fliSrcImage)).IsFail())
					break;
				if((res = flDepthImageReconstruction3D.SetDestinationHeightMapImage(ref fliDstImage)).IsFail())
					break;
				if((res = flDepthImageReconstruction3D.SetDestinationTextureImage(ref fliTxtImage)).IsFail())
					break;
				if((res = flDepthImageReconstruction3D.SetDestinationObject(ref floDstObject)).IsFail())
					break;

				if((res = flDepthImageReconstruction3D.SetBackgroundPageIndex(1.00)).IsFail())
					break;
				if((res = flDepthImageReconstruction3D.SetDifferentialMethod(CFLDepthImageReconstruction3D.EDifferentialMethod.DoGY)).IsFail())
					break;
				if((res = flDepthImageReconstruction3D.SetSigma1(1.00)).IsFail())
					break;
				if((res = flDepthImageReconstruction3D.SetSigma2(0.60)).IsFail())
					break;
				if((res = flDepthImageReconstruction3D.SetKernelBias(0.001)).IsFail())
					break;
				if((res = flDepthImageReconstruction3D.SetFMThreshold(0.0000025)).IsFail())
					break;
				if((res = flDepthImageReconstruction3D.SetTransitionGain(20.00)).IsFail())
					break;
				if((res = flDepthImageReconstruction3D.SetSigmaSpatial(11.00)).IsFail())
					break;
				if((res = flDepthImageReconstruction3D.SetSigmaRange(0.05)).IsFail())
					break;
				if((res = flDepthImageReconstruction3D.SetDirection(CFLDepthImageReconstruction3D.EDirection.BottomToTop)).IsFail())
					break;
				if((res = flDepthImageReconstruction3D.SetPixelAccuracy(0.10)).IsFail())
					break;
				if((res = flDepthImageReconstruction3D.SetDepthPitch(0.80)).IsFail())
					break;

				// 알고리즘 수행 // Execute the algorithm
				if((res = flDepthImageReconstruction3D.Execute()).IsFail())
				{
