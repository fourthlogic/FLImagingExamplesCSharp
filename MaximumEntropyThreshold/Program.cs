...
namespace MaximumEntropyThreshold
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				CResult eResult;
				// 이미지 로드 // Load image
				if((eResult = fliISrcImage.Load("../../ExampleImages/Threshold/Escherichia.flif")).IsFail())
				{
				...
				if((eResult = (MaximumEntropyThreshold.Execute())).IsFail())
				{
					ErrorPrint(eResult, "Failed to execute MaximumEntropyThreshold.");
					break;
				}
				...
				if((eResult = layer2.DrawTextImage(flpTemp, String.Format("Destination Image( {0} < threshold)", (int)mvThreshold.GetAt(0)), EColor.YELLOW, EColor.BLACK, 20)).IsFail())
					ErrorPrint(eResult, "Failed to draw text.\n");
