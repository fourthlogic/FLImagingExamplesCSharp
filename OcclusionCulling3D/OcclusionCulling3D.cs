	...
	class OcclusionCulling
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				if((eResult = floSourceObject.Load("../../ExampleImages/OcclusionCulling3D/CUBE.ply")).IsFail())
				{
				...
				// OcclusionCulling3D 객체 생성 // Create OcclusionCulling3D object
				COcclusionCulling3D occlusionCulling3D = new COcclusionCulling3D();

				// Source object 설정 // Set the source object
				occlusionCulling3D.SetSourceObject(ref floSourceObject);

				// Destination object 설정 // Set the destination object
				occlusionCulling3D.SetDestinationObject(ref floDestinationObject);

				TPoint3<float> tp3ViewPoint = new TPoint3<float>();
				tp3ViewPoint.x = -191.367416f;
				...
				occlusionCulling3D.SetCullingMethod(COcclusionCulling3D.ECullingMethod.RayCasting_Normal);

				// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
				if((eResult = occlusionCulling3D.Execute()).IsFail())
				{
					//ErrorPrint(eResult, "Failed to execute Occlusion Culling 3D.");
					break;
				}
				...
				if((eResult = view3DDst.PushObject((CFL3DObject)occlusionCulling3D.GetDestinationObject())).IsFail())
				{
