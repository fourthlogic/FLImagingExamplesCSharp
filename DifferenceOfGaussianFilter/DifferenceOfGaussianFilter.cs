...
{
	class DifferenceOfGaussianFilter
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				if((res = arrLayer[(int)EType.Destination1].DrawTextCanvas(flpZero, "Destination1 Image (Sigma 1, SigFactor 1.6)", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}

				if((res = arrLayer[(int)EType.Destination2].DrawTextCanvas(flpZero, "Destination2 Image (Sigma 2, SigFactor 1.6)", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}

				if((res = arrLayer[(int)EType.Destination3].DrawTextCanvas(flpZero, "Destination3 Image (Sigma 3, SigFactor 1.6)", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}

				if((res = arrLayer[(int)EType.Destination4].DrawTextCanvas(flpZero, "Destination4 Image (Sigma 3, SigFactor 3)", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}
