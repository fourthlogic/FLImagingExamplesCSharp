...
namespace DifferenceOfGaussianFilter
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			{
				// 이미지 로드 // Load image
                if ((eResult = arrFliImage[(int)EType.Source].Load("../../ExampleImages/DifferenceOfGaussianFilter/Gear.flif")).IsFail())
				{
				...
				{
					ErrorPrint(eResult, "Failed to execute DifferenceOfGaussianFilter.");
					break;
				}
				...
				if((eResult = arrLayer[(int)EType.Destination1].DrawTextCanvas(flpZero, "Destination1 Image (Sigma 1, SigFactor 1.6)", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = arrLayer[(int)EType.Destination2].DrawTextCanvas(flpZero, "Destination2 Image (Sigma 2, SigFactor 1.6)", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = arrLayer[(int)EType.Destination3].DrawTextCanvas(flpZero, "Destination3 Image (Sigma 3, SigFactor 1.6)", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = arrLayer[(int)EType.Destination4].DrawTextCanvas(flpZero, "Destination4 Image (Sigma 3, SigFactor 3)", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}
