...
{
	class OperationMatrixMultiply_Scalar
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				if((res = arrFliImage[(int)EType.Source].Load("../../ExampleImages/OperationMatrixMultiply/Matrix0.flif")).IsFail())
				{
				...
				operationMatrixMultiply.SetDestinationImage(ref arrFliImage[(int)EType.Destination1]);
				// 연산 방식 설정 // Set operation source
				operationMatrixMultiply.SetOperationSource(EOperationSource.Scalar);
				// Multiply 값 설정 // Set Multiply value
				CMultiVar<double> mvScalarValue1 = new CMultiVar<double>(1.5);
				...
				operationMatrixMultiply.SetDestinationImage(ref arrFliImage[(int)EType.Destination2]);
				// Multiply 값 설정 // Set Multiply value
				CMultiVar<double> mvScalarValue2 = new CMultiVar<double>(2);
				...
				if((res = (arrLayer[(int)EType.Destination1].DrawTextCanvas(flpZero, "Destination1 Image(Multiply 1.5)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}

				if((res = (arrLayer[(int)EType.Destination2].DrawTextCanvas(flpZero, "Destination2 Image(Multiply 2)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}
