	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				CResult eResult;
				// 이미지 로드 // Load image
				if((eResult = (arrFliImage[0].Load("../../ExampleImages/OperationMinimum/Flower.flif"))).IsFail())
				{
				...
				minimum.SetOperationSource(EOperationSource.Scalar);
				// Scalar 값 설정 // Set Scalar value
				minimum.SetScalarValue(mvScalr2);
				...
				if((eResult = (arrLayer[0].DrawTextCanvas(tpPosition, "Source Image", EColor.YELLOW, EColor.BLACK, 25))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = (arrLayer[1].DrawTextCanvas(tpPosition, "Destination1 Image(Minimum 100)", EColor.YELLOW, EColor.BLACK, 25))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = (arrLayer[2].DrawTextCanvas(tpPosition, "Destination2 Image(Minimum 200)", EColor.YELLOW, EColor.BLACK, 25))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}
