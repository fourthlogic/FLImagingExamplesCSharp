	...
	class OperationMinimum_Scalar
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				CResult res;
				// 이미지 로드 // Load image
				if((res = (arrFliImage[0].Load("../../ExampleImages/OperationMinimum/Flower.flif"))).IsFail())
				{
				...
				operationMinimum.SetDestinationImage(ref arrFliImage[1]);
				// 연산 방식 설정 // Set operation source
				operationMinimum.SetOperationSource(EOperationSource.Scalar);
				// Scalar 값 설정 // Set Scalar value
				operationMinimum.SetScalarValue(mvScalr);
				...
				operationMinimum.SetOperationSource(EOperationSource.Scalar);
				// Scalar 값 설정 // Set Scalar value
				operationMinimum.SetScalarValue(mvScalr2);
				...
				if((res = (arrLayer[1].DrawTextCanvas(tpPosition, "Destination1 Image(Minimum 100)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}

				if((res = (arrLayer[2].DrawTextCanvas(tpPosition, "Destination2 Image(Minimum 200)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}
