...
{
	class StatisticalOutlierRemoval3D
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				if((eResult = floSourceObject.Load("../../ExampleImages/StatisticalOutlierRemoval3D/Cube_Noisy.fl3do")).IsFail())
				{
				...
				statisticalOutlierRemoval3D.SetDestinationObject(ref floDestinationObject);
				// Source object 설정 // Set the source object
				statisticalOutlierRemoval3D.SetSourceObject(ref floSourceObject);
				// 탐색할 최근접 이웃 수 설정 // Set the number of nearest neighbors to search
				statisticalOutlierRemoval3D.SetNeighborCount(40);
				// 이상치 계수 설정 // Set the outlier coefficient
				...
				if((eResult = view3DDst.PushObject((CFL3DObject)statisticalOutlierRemoval3D.GetDestinationObject())).IsFail())
				{
