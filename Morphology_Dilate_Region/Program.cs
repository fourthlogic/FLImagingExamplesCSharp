	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				CMorphologyDilate morphologyDilate = new CMorphologyDilate();
				// Source 이미지 설정 // Set the source image
				morphologyDilate.SetSourceImage(ref arrFliImage[(int)EType.Source]);
				// Source ROI 설정 // Set the Source ROI
				morphologyDilate.SetSourceROI(flrROI);

				// Destination 이미지 설정 // Set the destination image
				morphologyDilate.SetDestinationImage(ref arrFliImage[(int)EType.Destination]);
				...
				morphologyDilate.SetKernel(flrRegion);

				// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
				if((eResult = (morphologyDilate.Execute())).IsFail())
				{
