        ...
        static void Main(string[] args)
		{
			...
			do
			{
				...
				for(Int64 i = 0; i < i64Results; ++i)
				{
					...
					if((res = dataMatrixVerifier.GetResultDataRegion(i, ref flqRegion)).IsFail())
					{
					...
					if((res = dataMatrixVerifier.GetResultGridRegion(i, ref flfaGridRegion)).IsFail())
					{
					...
					// Data Matrix Verifier 결과를 얻어오기 위해 string 선언
					StringBuilder strDecoded = new StringBuilder();

					// Data Matrix Verifier 결과들 중 Decoded String 을 얻어옴
					if((res = dataMatrixVerifier.GetResultDecodedString(i, ref strDecoded)).IsFail())
					{
					...
					if((res = dataMatrixVerifier.GetResultPrintQuality_ISOIEC_15415(i, ref printQuality)).IsFail())
					{
