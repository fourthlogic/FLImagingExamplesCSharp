	...
	class SurfaceMatch3DMulti
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				CVertexMatch3DMulti vertexMatch3DMulti = new CVertexMatch3DMulti();
				...
				for(int i = 0; i < 3; ++i)
				{
					...
					vertexMatch3DMulti.SetLearnObject(ref arrLearnObject[i]);

					// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
					if((eResult = vertexMatch3DMulti.Learn(arrClassName[i])).IsFail())
					{
						ErrorPrint(eResult, "Failed to learn Vertex Match 3D Multi.");
						break;
					}
				...
				vertexMatch3DMulti.SetSourceObject(ref floSourceObject);
				...
				{
					ErrorPrint(eResult, "Failed to execute Vertex Match 3D Multi.");
					break;
				}
				...
				long i64ResultCount = vertexMatch3DMulti.GetResultCount();
				...
				for(long i = 0; i < i64ResultCount; i++)
				{
					...
					// 추정된 포즈 행렬 가져오기
					if((eResult = vertexMatch3DMulti.GetResultPoseMatrix(i, ref sResult)).IsFail())
					{
						ErrorPrint(eResult, "Failed to estimate pose matrix.\n");
						break;
					}
					...
					if((eResult = vertexMatch3DMulti.GetResultObject(i, ref floLearnTransform, ref tp3Center)).IsFail())
					{
						ErrorPrint(eResult, "Failed to estimate pose matrix.\n");
						break;
					}
