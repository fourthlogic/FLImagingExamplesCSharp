	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				if((eResult = flcSource1.GetPointsOfMinimumDistance(flqOperand1, out flpaResult1)).IsFail())
				{
				...
				if((eResult = flfaSource2.GetPointsOfMinimumDistance(flfaOperand2, out flpaResult2)).IsFail())
				{
					ErrorPrint(eResult, "Failed to process.\n");
					break;
				}
				...
				if((eResult = flcSource1.GetMinimumDistance(flqOperand1, out f64MinimumDistance1)).IsFail())
				{
					ErrorPrint(eResult, "Failed to process.\n");
					break;
				}
				...
				if((eResult = flfaSource2.GetMinimumDistance(flfaOperand2, out f64MinimumDistance2)).IsFail())
				{
					ErrorPrint(eResult, "Failed to process.\n");
					break;
				}
