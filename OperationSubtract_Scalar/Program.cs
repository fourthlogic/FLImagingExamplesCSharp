	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				CResult eResult;
				// 이미지 로드 // Load image
				if((eResult = (arrFliImage[0].Load("../../ExampleImages/OperationSubtract/House.flif"))).IsFail())
				{
				...
				if((eResult = (arrLayer[1].DrawTextCanvas(tpPosition, "Destination1 Image(Sub 30, 0, 0)", EColor.YELLOW, EColor.BLACK, 25))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = (arrLayer[2].DrawTextCanvas(tpPosition, "Destination2 Image(Sub 0, 0, 30)", EColor.YELLOW, EColor.BLACK, 25))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}
