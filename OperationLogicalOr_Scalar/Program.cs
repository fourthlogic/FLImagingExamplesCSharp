    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
			...
			do
            {
                ...
                if ((res = fliSourceImage.Load("../../ExampleImages/OperationLogicalOr/Space.flif")).IsFail())
                {
                ...
                if ((res = layerSource.DrawTextCanvas(flpPoint, "Source Image", EColor.YELLOW, EColor.BLACK, 25)).IsFail())
                {
                    ErrorPrint(res, "Failed to draw text. \n");
                    break;
                }

                if ((res = layerDestination1.DrawTextCanvas(flpPoint, "Destination1 Image(LogicalOr 0)", EColor.YELLOW, EColor.BLACK, 25)).IsFail())
                {
                    ErrorPrint(res, "Failed to draw text. \n");
                    break;
                }

                if ((res = layerDestination2.DrawTextCanvas(flpPoint, "Destination2 Image(LogicalOr 255)", EColor.YELLOW, EColor.BLACK, 25)).IsFail())
                {
                    ErrorPrint(res, "Failed to draw text. \n");
                    break;
                }
