	...
	class MeasurementInstanceDL
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				// 이미지 로드 // Load image
				if((res = fliLearnImage.Load("../../ExampleImages/MeasurementDL/Cross.flif")).IsFail())
				{
					ErrorPrint(res, "Failed to load the image file. \n");
					break;
				}

				if((res = fliValidationImage.Load("../../ExampleImages/MeasurementDL/Cross.flif")).IsFail())
				{
					ErrorPrint(res, "Failed to load the image file.\n");
					break;
				}
				...
				// MeasurementInstance 객체 생성 // Create MeasurementInstance object
				CMeasurementInstanceDL measurementInstanceDL = new CMeasurementInstanceDL();

				// 학습할 이미지 설정 // Set the image to learn
				measurementInstanceDL.SetLearningImage(ref fliLearnImage);
				// 검증할 이미지 설정 // Set the image to validate
				measurementInstanceDL.SetLearningValidationImage(ref fliValidationImage);
				// 분류할 이미지 설정 // Set the image to classify
				measurementInstanceDL.SetInferenceImage(ref fliValidationImage);
				measurementInstanceDL.SetInferenceResultImage(ref fliResultLabelImage);

				// 학습 epoch 값을 설정 // Set the learn epoch value 
				measurementInstanceDL.SetLearningEpoch(1000);
				// 학습 이미지 Interpolation 방식 설정 // Set Interpolation method of learn image
				measurementInstanceDL.SetInterpolationMethod(EInterpolationMethod.Bilinear);
				...
				// 설정한 Optimizer를 measurementInstanceDL 적용 // Apply Optimizer that we set up to measurementInstanceDL
				measurementInstanceDL.SetLearningOptimizerSpec(optSpec);

				// 학습을 종료할 조건식 설정. map값이 1 이상인 경우 학습 종료한다. metric와 동일한 값입니다.
				// Set Conditional Expression to End Learning. If the map값이 value is 1 or higher, end the learning. Same value as metric.
				measurementInstanceDL.SetLearningStopCondition("map >= 1");

				CAugmentationSpecDL augSpec = new CAugmentationSpecDL();

				augSpec.EnableAugmentation(false);
				measurementInstanceDL.SetLearningAugmentationSpec(augSpec);
				...
				autoSaveSpec.SetAutoSavePath("model.flmsidl");
				...
				// 자동 저장 옵션 설정 // Set Auto-Save Options
				measurementInstanceDL.SetLearningAutoSaveSpec(autoSaveSpec);

				// MeasurementInstance learn function을 진행하는 스레드 생성 // Create the MeasurementInstance Learn function thread
				ThreadPool.QueueUserWorkItem((arg) =>
				{
					if((res = measurementInstanceDL.Learn()).IsFail())
				...
				while(!measurementInstanceDL.IsRunning() && !bTerminated)
					Thread.Sleep(1);

				int i32MaxEpoch = measurementInstanceDL.GetLearningEpoch();
				int i32PrevEpoch = 0;
				...
				while(true)
				{
					...
					int i32MiniBatchCount = measurementInstanceDL.GetActualMiniBatchCount();
					// 마지막 미니 배치 반복 횟수 받기 // Get the last number of mini batch iterations
					int i32Iteration = measurementInstanceDL.GetLearningResultCurrentIteration();
					// 마지막 학습 횟수 받기 // Get the last epoch learning
					int i32Epoch = measurementInstanceDL.GetLastEpoch();
					...
					if(i32Epoch != i32PrevEpoch && i32Iteration == i32MiniBatchCount && i32Epoch > 0)
					{
						...
						float f32CurrCost = measurementInstanceDL.GetLearningResultLastCost();
						// 마지막 검증 결과 받기 // Get the last validation result
						float f32MeanAP = measurementInstanceDL.GetLearningResultLastMeanAP();
						// 마지막 Recall 결과 받기 // Get the last recall result
						float f32Recall = measurementInstanceDL.GetLearningResultLastRecall();
						// 마지막 검증 결과 받기 // Get the last validation result
						float f32Precision = measurementInstanceDL.GetLearningResultLastPrecision();
						...
						measurementInstanceDL.GetLearningResultAllHistory(ref vctCosts, ref vctMeanAP, ref vctValidationEpoch, ref vctRecall, ref vctPrecision);
						...
						{
							int i32Step = measurementInstanceDL.GetLearningValidationStep();
					...
					if(!measurementInstanceDL.IsRunning())
						break;
				}
				...
				if((res = measurementInstanceDL.Execute()).IsFail())
				{
