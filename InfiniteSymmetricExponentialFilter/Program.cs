...
namespace InfiniteSymmetricExponentialFilter
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			{
				// 이미지 로드 // Load image
                if ((eResult = arrFliImage[(int)EType.Source].Load("../../ExampleImages/InfiniteSymmetricExponentialFilter/mountain.flif")).IsFail())
				{
				...
				{
					ErrorPrint(eResult, "Failed to execute InfiniteSymmetricExponential filter.");
					break;
				}
				...
				if((eResult = arrLayer[(int)EType.Destination1].DrawTextCanvas(flpZero, "Destination1 Image (PValue 0.3)", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = arrLayer[(int)EType.Destination2].DrawTextCanvas(flpZero, "Destination2 Image (PValue 0.6)", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = arrLayer[(int)EType.Destination3].DrawTextCanvas(flpZero, "Destination3 Image (PValue 0.9)", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = arrLayer[(int)EType.Destination4].DrawTextCanvas(flpZero, "Destination4 Image (PValue 1.5)", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}
