...
{
	class OperationScaledMultiply_Scalar
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				if((res = arrFliImage[(int)EType.Source].Load("../../ExampleImages/OperationScaledMultiply/CityNight.flif")).IsFail())
				{
				...
				operationScaledMultiply.SetDestinationImage(ref arrFliImage[(int)EType.Destination1]);
				...
				operationScaledMultiply.SetScalarValue(mvScalarValue1);
				...
				operationScaledMultiply.SetDestinationImage(ref arrFliImage[(int)EType.Destination2]);
				// ScaledMultiply 값 설정 // Set ScaledMultiply value
				CMultiVar<double> mvScalarValue2 = new CMultiVar<double>(430, 430, 430);
				operationScaledMultiply.SetScalarValue(mvScalarValue2);
				...
				if((res = (arrLayer[(int)EType.Destination1].DrawTextCanvas(flpZero, "Destination1 Image(ScaledMultiply 128)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}

				if((res = (arrLayer[(int)EType.Destination2].DrawTextCanvas(flpZero, "Destination2 Image(ScaledMultiply 430)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}
