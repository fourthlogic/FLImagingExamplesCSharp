...
namespace OperationMultiply_Scalar
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				if((eResult = arrFliImage[(int)EType.Source].Load("../../ExampleImages/OperationScaledMultiply/CityNight.flif")).IsFail())
				{
				...
				scaledMultiply.SetDestinationImage(ref arrFliImage[(int)EType.Destination1]);
				// 연산 방식 설정 // Set operation source
				scaledMultiply.SetOperationSource(EOperationSource.Scalar);
				// ScaledMultiply 값 설정 // Set ScaledMultiply value
				CMultiVar<double> mvScalarValue1 = new CMultiVar<double>(128, 128, 128);
				scaledMultiply.SetScalarValue(mvScalarValue1);

				// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
				if((eResult = (scaledMultiply.Execute())).IsFail())
				{
				...
				scaledMultiply.SetDestinationImage(ref arrFliImage[(int)EType.Destination2]);
				// ScaledMultiply 값 설정 // Set ScaledMultiply value
				CMultiVar<double> mvScalarValue2 = new CMultiVar<double>(430, 430, 430);
				scaledMultiply.SetScalarValue(mvScalarValue2);
				...
				if((eResult = (arrLayer[(int)EType.Destination1].DrawTextCanvas(flpZero, "Destination1 Image(ScaledMultiply 128)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = (arrLayer[(int)EType.Destination2].DrawTextCanvas(flpZero, "Destination2 Image(ScaledMultiply 430)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}
