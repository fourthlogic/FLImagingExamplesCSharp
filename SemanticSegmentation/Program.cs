	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				SemanticSegmentation.SetLearnImage(ref fliLearnImage);
				// 검증할 이미지 설정 // Set the image to validate
				SemanticSegmentation.SetLearnValidationImage(ref fliValidationImage);
				...
				int i32MaxEpoch = SemanticSegmentation.GetLearnEpoch();
				int i32PrevEpoch = 0;
				...
				while(true)
				{
					...
					if(i32Epoch != i32PrevEpoch && i32Iteration == i32MaxIteration && i32Epoch > 0)
					{
						...
						float f32CurrCost;
						SemanticSegmentation.GetLastCost(out f32CurrCost);
						// 마지막 검증 결과 받기 // Get the last validation result
						float f32ValidationPa = 0;
						float f32ValidationPaMIoU = 0;
						SemanticSegmentation.GetLastValidationPixelAccuracy(out f32ValidationPa);//SemanticSegmentation.GetLastValidationPixelAccuracy(out f32ValidationPa);
						SemanticSegmentation.GetLastValidationMIoU(out f32ValidationPaMIoU);//SemanticSegmentation.GetLastValidationMIoU(out f32ValidationPaMIoU);
						...
						SemanticSegmentation.GetCostHistory(out vctCosts);
						SemanticSegmentation.GetValidationHistory(out vctValidations);
						SemanticSegmentation.GetMIoUHistory(out vctMIoU);
						...
						{
							int i32Step = SemanticSegmentation.GetLearnValidationStep();
				...
				SemanticSegmentation.GetInferenceResultLabelImage(out fliResultLabelImage);
				...
				blob.SetLogicalCondition(ELogicalCondition.Equal);
				blob.SetThresholdMode(EThresholdMode.Single);
				blob.SetResultType(CBlob.EBlobResultType.Contour);
				blob.SetContourResultType(CBlob.EContourResultType.Perforated);
				blob.SetSourceImage(ref fliResultLabelFigureImage);

				// Blob figure 결과를 이미지에 붙여넣기 // Paste blob's results to the result blob image
				long i64LoopCount = SemanticSegmentation.GetLearntResultLabelCategoryCount();

				for(long i = 0; i < i64LoopCount; ++i)
				{
					blob.SetThreshold(i);
					blob.Execute();

					if((eResult = blob.Execute()).IsFail())
					{
					...
					if((eResult = blob.GetResultContours(out flfaContours)).IsFail())
					{
						ErrorPrint(eResult, "Failed to process\n");
						break;
					}
					...
					int i32RealClassNum = (int)i;
					SemanticSegmentation.GetLearntResultLabelCategoryClassNames(i, out flaNames);

					string flsLabel = string.Format("{0}({1})", i32RealClassNum, flaNames[0]);

					for(int j = 0; j < flfaContours.GetCount(); ++j)
					{
						CFLFigure flrg = flfaContours.GetAt(j);

						flrg.SetName(flsLabel);
						fliResultLabelFigureImage.PushBackFigure(CROIUtilities.ConvertFigureObjectToString(flrg));
					}
				}

				// ResultLabl 뷰에 Floating Value Range를 설정
				viewImageResultLabel.SetFloatingImageValueRange(0, (float)SemanticSegmentation.GetLearntResultLabelCategoryCount());
