	...
	class FLDenoisingType2
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				// FL Denoising Type 2 객체 생성 // Create FL Denoising Type 2 object
				CFLDenoisingType2 flDenoisingType2 = new CFLDenoisingType2();

				// Source 이미지 설정 // Set source image 
				if((res = flDenoisingType2.SetSourceImage(ref fliSourceImage)).IsFail())
				{
				...
				if((res = flDenoisingType2.SetDestinationImage(ref fliConvertedImage)).IsFail())
				{
				...
				if((res = (flDenoisingType2.SetKernel(3))).IsFail())
				{
				...
				if((res = (flDenoisingType2.SetAmplitude(0.2))).IsFail())
				{
				...
				if((res = (flDenoisingType2.SetOperationMode(CFLDenoisingType2.EOperationMode.Fast))).IsFail())
				{
				...
				if((res = (flDenoisingType2.Execute())).IsFail())
				{
