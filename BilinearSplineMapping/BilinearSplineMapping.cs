	...
	class BilinearSplineMapping
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				CBilinearSplineMapping bilinearSplineMapping = new CBilinearSplineMapping();
				...
				if((res = (bilinearSplineMapping.Load("MappingData.flbls"))).IsFail())
				{
					...
					// If you try to convert the value outside the extended range, it will result in inaccurate data,
				...
				for(int y = 0; y < bilinearSplineMapping.GetRow(); ++y)
				{
					for(int x = 0; x < bilinearSplineMapping.GetColumn(); ++x)
					{
						CBilinearSplineMapping.CBilinearSplineMappingVertexInfo vertex = bilinearSplineMapping.GetControlPoint(new CFLPoint<int>(x, y));
				...
				for(int y = 0; y <= (bilinearSplineMapping.GetRow() - 1) * f64Slice; ++y)
				{
					...
					for(int x = 0; x <= (bilinearSplineMapping.GetColumn() - 1) * f64Slice; ++x)
					{
						...
						if(bilinearSplineMapping.ConvertSourceToDestination(flpdSource, ref flpdDestination).IsOK())
						{
							...
							if(bilinearSplineMapping.ConvertDestinationToSource(flpdDestination, ref flpdConvertedSource).IsOK())
							{
