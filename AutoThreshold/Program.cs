...
namespace Statistics
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			{
				// 이미지 로드 // Load image
				if((eResult = arrFliImage[(int)EType.Source].Load("../../ExampleImages/Threshold/Mountain.flif")).IsFail())
				{
				...
				CFLRect<int> flrROI = new CFLRect<int>(300, 200, 700, 600);

				// gauss filter 객체 생성 // Create gauss filter object
				CAutoThreshold autothreshold = new CAutoThreshold();
				// Source 이미지 설정 // Set the source image
				autothreshold.SetSourceImage(ref arrFliImage[(int)EType.Source]);
				// Source ROI 설정 // Set the Source ROI
				autothreshold.SetSourceROI(flrROI);

				// Destination1 이미지 설정 // Set the destination1 image
				autothreshold.SetDestinationImage(ref arrFliImage[(int)EType.Destination1]);
				...
				{
					ErrorPrint(eResult, "Failed to execute gauss filter.");
					break;
				}

				// Result 결과 갯수 확인 // get result count
				Int64 i64IndexCount = autothreshold.GetResultBatchCount();

				// Threshold 값 표기를 위한 String 변수 // string variable to indicate Threshold value
				List<string> strThreshold1 = new List<string>();

				// AutoThreshold 결과값 // auto threshold result object
				List<int> listResult1 = new List<int>();
				...
				for(Int64 i = 0; i < i64IndexCount; ++i)
				{
					...
					Int64 i64ChannelCount = autothreshold.GetChannelCount(i);
					...
					for(int j = 0; j < i64ChannelCount; ++j)
					{
						...
						if((eResult = (autothreshold.GetResultBatch(i, j, out listResult1))).IsFail())
						{
							Console.WriteLine("No Result.");
							break;
						}

						Int64 i64ListCount = listResult1.Count();

						// Threshold 값 표기를 위한 String 임시변수 // string variable to indicate threshold temporary value
						string strTemp = String.Format("Ch{0} ", j);

						for(int k = 0; k < i64ListCount; ++k)
						{
							strTemp += String.Format("{0} ", listResult1[k]);
						}

						strThreshold1.Add(strTemp);
					}
				}

				// Destination2 이미지 설정 // Set the destination2 image
				autothreshold.SetDestinationImage(ref arrFliImage[(int)EType.Destination2]);
				...
				// Threshold 값 표기를 위한 String 변수 // string variable to indicate Threshold value
				List<string> strThreshold2 = new List<string>();

				// AutoThreshold 결과값 // auto threshold result object
				listResult1 = new List<int>();
				...
				CFLPoint<double> flpThreshold = new CFLPoint<double>(20, 320);
				...
				if((eResult = arrLayer[(int)EType.Destination1].DrawTextCanvas(flpZero, "Destination1 Image (Sigma 2)", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = arrLayer[(int)EType.Destination1].DrawTextCanvas(flpThreshold, strThreshold1[0], EColor.YELLOW, EColor.BLACK, 17)).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = arrLayer[(int)EType.Destination2].DrawTextCanvas(flpZero, "Destination2 Image (Sigma 4)", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = arrLayer[(int)EType.Destination2].DrawTextCanvas(flpThreshold, strThreshold2[0], EColor.YELLOW, EColor.BLACK, 17)).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}
