	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				layer[1].DrawTextCanvas(new CFLPoint<int>(0, 0), "Close Result1", EColor.YELLOW, EColor.BLACK, 30);
				layer[2].DrawTextCanvas(new CFLPoint<int>(0, 0), "Close Result2", EColor.YELLOW, EColor.BLACK, 30);
				layer[3].DrawTextCanvas(new CFLPoint<int>(0, 0), "Close Result2", EColor.YELLOW, EColor.BLACK, 30);
				...
				if((res = flrgSourceFig.Close(i64KernelSize, i64KernelSize, ref flfaResult1)).IsFail())
				{
				...
				if((res = flrgSourceFig.Close(i64KernelSize, i64KernelSize, ref flfaResult2, EKernelShape.Circle)).IsFail())
				{
					ErrorPrint(res, "Failed to calculate.\n");
					break;
				}
				...
				if((res = flrgSourceFig.Close(fleKernel, ref flfaResult3)).IsFail())
				{
					ErrorPrint(res, "Failed to calculate.\n");
					break;
				}
