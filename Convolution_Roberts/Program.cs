...
namespace Convolution_Roberts
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			{
				// 이미지 로드 // Load image
				if((eResult = arrFliImage[(int)EType.Source].Load("../../ExampleImages/Convolution/Sunset.flif")).IsFail())
				{
				...
				CConvolutionRoberts convolutionRoberts = new CConvolutionRoberts();
				// Source 이미지 설정 // Set the source image
				convolutionRoberts.SetSourceImage(ref arrFliImage[(int)EType.Source]);
				// Source ROI 설정 // Set the Source ROI
				convolutionRoberts.SetSourceROI(flrROI);

				// Destination1 이미지 설정 // Set the destination1 image
				convolutionRoberts.SetDestinationImage(ref arrFliImage[(int)EType.Destination1]);
				// Destination1 ROI 설정 // Set the destination1 ROI
				convolutionRoberts.SetDestinationROI(flrROI);

				// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
				if((eResult = (convolutionRoberts.Execute())).IsFail())
				{
				...
				if((eResult = (arrLayer[(int)EType.Destination1].DrawTextCanvas(flpZero, "Destination1 Image (Roberts)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}
