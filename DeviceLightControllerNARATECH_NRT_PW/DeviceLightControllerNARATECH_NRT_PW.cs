...
{
	class DeviceLightControllerNARATECH_NRT_PW
	{
		...
		static void Main(string[] args)
		{
			...
			CDeviceLightControllerNARATECH_NRT_PW lightControllerNARATECH_NRT_PW = new CDeviceLightControllerNARATECH_NRT_PW();
			...
			do
			{
				...
				if(int.TryParse(Console.ReadLine(), out var portNumber))
				{
					...
					lightControllerNARATECH_NRT_PW.SetComPortNumber(portNumber);
				}

				Console.Clear();
				Console.Write("BaudRate(Switch ON = 9600, OFF = 19200]: ");

				if(int.TryParse(Console.ReadLine(), out var baudRate))
				{
					// 보드레이트 설정 // Set the baud rate.
					lightControllerNARATECH_NRT_PW.SetBaudRate(baudRate);
				}

				if(lightControllerNARATECH_NRT_PW.Initialize().IsFail())
				{
					...
					if(int.TryParse(Console.ReadLine(), out var channel))
					{
						if(operationMode == 1)
						{
							...
							if(int.TryParse(Console.ReadLine(), out var onOff))
							{
								...
								lightControllerNARATECH_NRT_PW.SetChannelState(channel, onOff == 1);
							}
						}
						else if(operationMode == 2)
						{
							...
							if(int.TryParse(Console.ReadLine(), out var lightValue))
							{
								...
								lightControllerNARATECH_NRT_PW.SetLightValue(channel, (byte)lightValue);
							}
						}
					}
			...
			if(lightControllerNARATECH_NRT_PW.Terminate().IsFail())
			{
