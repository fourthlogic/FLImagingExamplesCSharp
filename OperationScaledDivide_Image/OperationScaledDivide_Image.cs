	...
	class OperationScaledDivide_Image
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				if((res = operationScaledDivide.SetOperandImage(ref fliOperandImage)).IsFail())
				{
				...
				if((res = operationScaledDivide.SetDestinationImage(ref fliDestinationImage)).IsFail())
				{
				...
				if((res = layerOperand.DrawTextCanvas(new CFLPoint<double>(0, 0), "Operand Image", EColor.YELLOW, EColor.BLACK, 18)).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}

				if((res = layerDestination.DrawTextCanvas(new CFLPoint<double>(0, 0), "Destination Image", EColor.YELLOW, EColor.BLACK, 18)).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}
