	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				if((eResult = arrFliImage[(int)EType.Source].Load("../../ExampleImages/OperationScaledDivide/Generator.flif")).IsFail())
				{
				...
				if((eResult = arrFliImage[(int)EType.Operand].Load("../../ExampleImages/OperationScaledDivide/Gradation_R2W.flif")).IsFail())
				{
					ErrorPrint(eResult, "Failed to load the image file.\n");
					break;
				}
				...
				if((eResult = (scaledDivide.Execute())).IsFail())
				{
				...
				if((eResult = (arrLayer[(int)EType.Source].DrawTextCanvas(flpZero, "Source Image", EColor.YELLOW, EColor.BLACK, 18))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = (arrLayer[(int)EType.Operand].DrawTextCanvas(flpZero, "Operand Image", EColor.YELLOW, EColor.BLACK, 18))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = (arrLayer[(int)EType.Destination].DrawTextCanvas(flpZero, "Destination Image", EColor.YELLOW, EColor.BLACK, 18))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}
