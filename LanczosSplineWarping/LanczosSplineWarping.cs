	...
	class LanczosSplineWarping
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				if((res = arrFliImage[0].Load("../../ExampleImages/LanczosWarping/chess.flif")).IsFail())
				{
				...
				if((arrViewImage[2].Create(100, 512, 612, 1024)).IsFail())
				{
					ErrorPrint(res, "Failed to create the image view.\n");
					break;
				}
				...
				if((arrViewImage[3].Create(612, 512, 1124, 1024)).IsFail())
				{
					ErrorPrint(res, "Failed to create the image view.\n");
					break;
				}
				...
				// Destination 이미지가 새로 생성됨으로 Zoom fit 을 통해 디스플레이 되는 이미지 배율을 화면에 맞춰준다. // With the newly created Destination image, the image magnification displayed through Zoom fit is adjusted to the screen.
				if((res = arrViewImage[3].ZoomFit()).IsFail())
				{
					ErrorPrint(res, "Failed to zoom fit of the image view.\n");
					break;
				}
