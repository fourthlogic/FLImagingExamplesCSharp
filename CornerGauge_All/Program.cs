		...
		static void Main(string[] args)
        {
            ...
            do
            {
                ...
                if (res.IsOK())
                {
					...
					CornerGauge.GetMeasuredObject(out flfaResultCorners);

					layer.DrawFigureImage(flfaResultCorners, EColor.BLACK, 3);
					layer.DrawFigureImage(flfaResultCorners, EColor.CYAN, 1);

					for(int i = 0; i < flfaResultCorners.GetCount(); ++i)
						Console.WriteLine("#{0} : Corner X : {1}, Corner Y : {2}", i, flfaResultCorners.GetAt(i).GetCenter().x, flfaResultCorners.GetAt(i).GetCenter().y);
