...
namespace FLImagingExamplesCSharp
{
	class ModelBasedQRCodeDecoder
	{
		...
		static void Main(string[] args)
		{
			...
			{
				// 이미지 로드 // Loads image
				if((res = arrFliImage[(int)EViewList.Learn].Load("../../ExampleImages/QRCode/Learn.flif")).IsFail())
				{
					ErrorPrint(res, "Failed to load the image file.\n");
					break;
				}

				if((res = arrFliImage[(int)EViewList.ModelBaseFind].Load("../../ExampleImages/QRCode/Find.flif")).IsFail())
				{
					ErrorPrint(res, "Failed to load the image file.\n");
					break;
				}

				if((res = arrFliImage[(int)EViewList.NormalFind].Load("../../ExampleImages/QRCode/Find.flif")).IsFail())
				{
					ErrorPrint(res, "Failed to load the image file.\n");
					break;
				}
				...
				CModelBasedQRCodeDecoder modelBasedQRCodeDecoder = new CModelBasedQRCodeDecoder();

				// 학습 이미지 설정 // Sets the learn image.
				modelBasedQRCodeDecoder.SetLearnImage(ref arrFliImage[(int)EViewList.Learn]);
				...
				modelBasedQRCodeDecoder.SetSourceImage(ref arrFliImage[(int)EViewList.ModelBaseFind]);
				...
				CModelBasedQRCodeDecoder.CQRCodeLearnInformation qrcodeLearnInfo = new CModelBasedQRCodeDecoder.CQRCodeLearnInformation();
				modelBasedQRCodeDecoder.GetLearnResult(ref qrcodeLearnInfo);
				{
					Console.Write("\n[Model Based Learn Result]\n");

					CFLQuad<double> flqLearnedCodeRegion = qrcodeLearnInfo.decodedQRCodeInformation.pFlqRegion;
					string flsLearnedCode = qrcodeLearnInfo.decodedQRCodeInformation.pStrCode;
					...
					Console.Write("Code Version : {0}\n", qrcodeLearnInfo.i32QRVersion);

					string flsCodeColor = "";

					if(qrcodeLearnInfo.eColor == EDataCodeColor.BlackOnWhite)
						flsCodeColor = "Black On White";
					else if(qrcodeLearnInfo.eColor == EDataCodeColor.WhiteOnBlack)
						flsCodeColor = "White On Black";

					Console.Write("Code Color : {0}\n", flsCodeColor);

					string flsFlip = qrcodeLearnInfo.bFlip ? "Yes" : "No";
				...
				long i64Results = modelBasedQRCodeDecoder.GetResultCount();
				...
				for(long i = 0; i < i64Results; ++i)
				{
					...
					if((res = modelBasedQRCodeDecoder.GetResultDataRegion(i, ref flqDecodedCodeRegion)).IsFail())
					{
					...
					if((res = modelBasedQRCodeDecoder.GetResultDecodedString(i, ref flsDecodedCode)).IsFail())
					{
				...
				// 동작 이미지 설정 // Set source image
				qrcodeDecoder.SetSourceImage(ref arrFliImage[(int)EViewList.NormalFind]);
				// 코드 색상 설정 // Sets the code color.
				...
				for(long i = 0; i < i64Results; ++i)
				{
					...
					// QR Code Decoder 결과들 중 Data Region 을 얻어옴 // Gets the Data Region from the results of the QR Code decoder.
					if((res = qrcodeDecoder.GetResultDataRegion(i, ref flqDecodedCodeRegion)).IsFail())
					{
						ErrorPrint(res, "Failed to get data region from the QR code decoder object.");
						continue;
					}
					...
					// QR Code Decoder 결과들 중 Decoded String 을 얻어옴 // Gets the decoded string from the results of the QR Code decoder.
					if((res = qrcodeDecoder.GetResultDecodedString(i, ref flsDecodedCode)).IsFail())
					{
						ErrorPrint(res, "Failed to get decoded string from the QR code decoder object.");
						continue;
					}
