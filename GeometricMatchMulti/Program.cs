		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				for(long i64DataIdx = 0; i64DataIdx < 3; ++i64DataIdx)
				{
					...
					FLGeometricMatchMulti.SetLearnThresholdCoeff(1.0);
					...
					FLGeometricMatchMulti.EnablePreprocessing(false);
					...
					if((eResult = layerLearn.DrawFigureImage(learnRegion, arrColor[i64DataIdx])).IsFail())
						break;
					...
					FLGeometricMatchMulti.GetLearntFeature(out flfaFeaturePoints);
				...
				// 이미지 로드 // Load image
				if((eResult = fliFindImage.Load("../../ExampleImages/Matching/Geometric Mult Find.flif")).IsFail())
					break;
				...
				FLGeometricMatchMulti.SetConstrastOption(FLImagingCLR.AdvancedFunctions.EGeometricMatchMultiConstrastOption.Normal);
				...
				FLGeometricMatchMulti.SetFindThresholdCoeff(1.0);
				...
				// 알고리즘 수행 // Execute the Algoritm
				if((eResult = FLGeometricMatchMulti.Execute()).IsFail())
				{
				...
				for(long i = 0; i < i64ResultCount; ++i)
				{
					...
					FLGeometricMatchMulti.GetResult(i, out results);
					FLGeometricMatchMulti.GetResultForDetectedFeature(i, out flfaResultPoints);
					...
					if((eResult = layerFind.DrawFigureImage(flfaResultPoints, arrColor[i64Idx])).IsFail())
						break;
