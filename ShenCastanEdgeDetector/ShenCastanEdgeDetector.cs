	...
	class ShenCastanEdgeDetector
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				CShenCastanEdgeDetector shenCastanEdgeDetector = new CShenCastanEdgeDetector();

				// Source 이미지 설정 // Set source image 
				shenCastanEdgeDetector.SetSourceImage(ref fliISrcImage);

				// Destination 이미지 설정 // Set destination image
				shenCastanEdgeDetector.SetDestinationImage(ref fliIDstImage);

				// Preprocessing 전처리 설정 // Set Preprocessing preprocessing
				shenCastanEdgeDetector.SetSmoothingMode(CShenCastanEdgeDetector.ESmoothingMode.ISEF);
				...
				shenCastanEdgeDetector.SetThreshold(mvThresholdValue);

				// p 값 설정 // Set p value
				shenCastanEdgeDetector.SetPValue(0.5);

				// 알고리즘 수행 // Execute the algorithm
				if((res = (shenCastanEdgeDetector.Execute())).IsFail())
				{
