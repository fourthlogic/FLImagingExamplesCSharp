	...
	class GeometricMatch_SaveLoad
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				CGeometricMatch geometricMatchSave = new CGeometricMatch();
				CGeometricMatch geometricMatchLoad = new CGeometricMatch();

				// 학습할 이미지 설정 // Set the image to learn
				geometricMatchSave.SetLearnImage(ref fliLearnImage);
				...
				geometricMatchSave.SetLearnROI(learnRegion);
				geometricMatchSave.SetLearnPivot(flpLearnPivot);

				// 학습 파라미터를 설정합니다. // Set the learning parameters.
				// 추출할 특징점 개수를 설정합니다. // Set the number of feature points to be extracted.
				geometricMatchSave.SetFeatureCount(2048);
				...
				geometricMatchSave.SetFeatureFiltering(0.5);
				// 추출할 특징점 처리과정에서의 허용 임계값을 설정합니다. // Set the allowable threshold in the feature point processing process to be extracted.
				geometricMatchSave.SetLearnThresholdCoefficient(1.0);

				// 알고리즘 수행 // Execute the Algoritm
				if((res = geometricMatchSave.Learn()).IsFail())
				{
				...
				if((res = geometricMatchSave.Save("../../ExampleImages/Matching/Geometric Single Learn")).IsFail())
				{
				...
				if((res = geometricMatchSave.GetLearnedFeature(ref flfaFeaturePoints)).IsFail())
				{
				...
				if((res = geometricMatchLoad.Load("../../ExampleImages/Matching/Geometric Single Learn")).IsFail())
				{
				...
				geometricMatchLoad.SetSourceImage(ref fliFindImage);
				...
				geometricMatchLoad.SetAngleBias(0.0);
				...
				long i64ResultCount = geometricMatchLoad.GetResultCount();
				...
				for(long i = 0; i < i64ResultCount; ++i)
				{
					...
					geometricMatchLoad.GetResult(i, ref results);
					geometricMatchLoad.GetResultDetectedFeature(i, ref flfaResultPoints);
