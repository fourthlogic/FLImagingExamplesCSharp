...
namespace FLImagingExamplesCSharp
{
	...
	class DeviceCameraFrameGrabberHIK
	{
		...
		static void Main(string[] args)
		{
			...
			// Frame Grabber HIK 카메라 선언 // Declare Frame Grabber HIK camera

			CDeviceCameraFrameGrabberHIK camHIK = new CDeviceCameraFrameGrabberHIK();

			do
			{
				String strInput = "";

				int i32InterfaceIndex = 0;
				int i32DeviceIndex = 0;

				// 인터페이스 인덱스를 입력합니다. // Enter the interface index.
				Console.Write("Enter interface index: ");
				strInput = Console.ReadLine();

				if(Int32.TryParse(strInput, out i32InterfaceIndex) == false)
					i32InterfaceIndex = 0;

				// 장치 인덱스를 입력합니다. // Enter the device index.
				...
				if((res = camHIK.Initialize()).IsFail())
				{
