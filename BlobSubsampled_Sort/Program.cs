	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				// 이미지 로드 // Load image
				if((eResult = fliImage.Load("../../ExampleImages/Blob/Ball.flif")).IsFail())
				{
				...
				if((eResult = viewImageRecover.Create(968, 0, 1736, 576)).IsFail())
				{
					ErrorPrint(eResult, "Failed to create the image view.\n");
					break;
				}
				...
				if((eResult = viewImageRecover.SetImagePtr(ref fliImage)).IsFail())
				{
					ErrorPrint(eResult, "Failed to set image object on the image view.\n");
					break;
				}
				...
				if((eResult = viewImage.SynchronizePointOfView(ref viewImageRecover)).IsFail())
				{
				...
				if((eResult = viewImageRecover.ZoomFit()).IsFail())
				{
					ErrorPrint(eResult, "Failed to zoom fit\n");
					break;
				}
				...
				if((eResult = sBlob.Filter(CBlob.EFilterItem.BoundaryRectWidth, 50, ELogicalCondition.GreaterEqual)).IsFail())
				{
				...
				if((eResult = sBlob.Filter(CBlob.EFilterItem.BoundaryRectHeight, 50, ELogicalCondition.GreaterEqual)).IsFail())
				{
				...
				// 면적이 50보다 작은 객체들을 제거
				if((eResult = sBlob.Filter(CBlob.EFilterItem.Area, 50, ELogicalCondition.LessEqual)).IsFail())
				{
					ErrorPrint(eResult, "Blob filtering algorithm error occurred.");
					break;
				}
				...
				// Blob 결과를 정렬
				if((eResult = sBlob.Sort(flaItem, flaOrder)).IsFail())
				{
				...
				if((eResult = sBlob.GetResultBoundaryRects(out flfaSortedBoundaryRects)).IsFail())
				{
				...
				if((eResult = sBlob.Sort(CBlob.EFilterItem.Unselected, CBlob.EOrder.Ascending)).IsFail())
				{
					ErrorPrint(eResult, "Failed to sort from the Blob object.");
					break;
				}
				...
				if((eResult = sBlob.GetResultBoundaryRects(out flfaRecoverBoundaryRects)).IsFail())
				{
					ErrorPrint(eResult, "Failed to get boundary rects from the Blob object.");
					break;
				}
				...
				if((eResult = layer.DrawTextCanvas(flp, ("Sort"), EColor.YELLOW, EColor.BLACK, 30)).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text on the image view.\n");
					break;
				}

				if((eResult = layerRecover.DrawTextCanvas(flp, ("No Sort"), EColor.YELLOW, EColor.BLACK, 30)).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text on the image view.\n");
					break;
				}
				...
				if((eResult = layerRecover.DrawFigureImage(flrROI, EColor.BLUE)).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw figures objects on the image view.\n");
					break;
				}
				...
				if((eResult = layer.DrawFigureImage(flfaSortedBoundaryRects, EColor.RED, 1, EColor.RED, EGUIViewImagePenStyle.Solid, 1.0f, .25f)).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw figure objects on the image view.\n");
					break;
				}

				if((eResult = layerRecover.DrawFigureImage(flfaRecoverBoundaryRects, EColor.RED, 1, EColor.RED, EGUIViewImagePenStyle.Solid, 1.0f, .25f)).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw figure objects on the image view.\n");
					break;
				}
				...
				{
					CFLRect<double> pFlrRect = (CFLRect<double>)flfaSortedBoundaryRects.GetAt(i);
					...
					pFlrRect = (CFLRect<double>)flfaRecoverBoundaryRects.GetAt(i);
