...
namespace Convolution_Uniform
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			{
				// 이미지 로드 // Load image
				if((eResult = arrFliImage[(int)EType.Source].Load("../../ExampleImages/Convolution/SilverGrass.flif")).IsFail())
				{
				...
				CConvolutionUniform convolutionUniform = new CConvolutionUniform();
				// Source 이미지 설정 // Set the source image
				convolutionUniform.SetSourceImage(ref arrFliImage[(int)EType.Source]);
				// Source ROI 설정 // Set the Source ROI
				convolutionUniform.SetSourceROI(flrROI);

				// Destination1 이미지 설정 // Set the destination1 image
				convolutionUniform.SetDestinationImage(ref arrFliImage[(int)EType.Destination1]);
				...
				if((eResult = (convolutionUniform.Execute())).IsFail())
				{
				...
				convolutionUniform.SetDestinationImage(ref arrFliImage[(int)EType.Destination2]);
				...
				convolutionUniform.SetDestinationImage(ref arrFliImage[(int)EType.Destination3]);
				...
				convolutionUniform.SetDestinationImage(ref arrFliImage[(int)EType.Destination4]);
				...
				if((eResult = (arrLayer[(int)EType.Destination1].DrawTextCanvas(flpZero, "Destination1 Image (Kernel Size 3x3)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = (arrLayer[(int)EType.Destination2].DrawTextCanvas(flpZero, "Destination2 Image (Kernel Size 5x5)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = (arrLayer[(int)EType.Destination3].DrawTextCanvas(flpZero, "Destination3 Image (Kernel Size 7x7)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = (arrLayer[(int)EType.Destination4].DrawTextCanvas(flpZero, "Destination4 Image (Kernel Size 15x15)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}
