...
namespace Match
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			CPatternMatchMultiSparse FLPatternMatchMultiSparse = new CPatternMatchMultiSparse();
			...
			do
			{
				...
				for(long i64DataIdx = 0; i64DataIdx < 2; ++i64DataIdx)
				{
					...
					FLPatternMatchMultiSparse.SetLearnImage(ref fliLearnImage[i64DataIdx]);
					...
					if((FLPatternMatchMultiSparse.Learn(arrClassName[i64DataIdx])).IsFail())
						break;
				...
				FLPatternMatchMultiSparse.SetSourceImage(ref fliFindImage);
				...
				// 알고리즘 수행 // Execute the Algoritm
				if((eResult = FLPatternMatchMultiSparse.Execute()).IsFail())
				{
				...
				long i64ResultCount = FLPatternMatchMultiSparse.GetResultCount();
