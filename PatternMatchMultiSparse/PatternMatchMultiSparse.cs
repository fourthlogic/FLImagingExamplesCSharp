...
namespace FLImagingExamplesCSharp
{
	class PatternMatchMultiSparse
	{
		...
		static void Main(string[] args)
		{
			...
			CPatternMatchMultiSparse patternMatchMultiSparse = new CPatternMatchMultiSparse();
			...
			do
			{
				...
				for(long i64DataIdx = 0; i64DataIdx < 2; ++i64DataIdx)
				{
					...
					patternMatchMultiSparse.SetLearnImage(ref fliLearnImage[i64DataIdx]);
					...
					if((patternMatchMultiSparse.Learn(arrClassName[i64DataIdx])).IsFail())
					{
				...
				patternMatchMultiSparse.SetSourceImage(ref fliFindImage);
				...
				long i64ResultCount = patternMatchMultiSparse.GetResultCount();
				...
				for(long i = 0; i < i64ResultCount; ++i)
				{
					...
					patternMatchMultiSparse.GetResult(i, ref results);
