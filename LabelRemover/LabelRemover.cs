	...
	class LabelRemover
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
                ...
                if ((res = viewImagesResultRegex.SetImagePtr(ref fliResultRegexImage)).IsFail())
                {
                    ErrorPrint(res, "Failed to set image object on the image view. \n");
                    break;
                }
                ...
                if ((res = viewImageSource.SynchronizeWindow(ref viewImagesResultRegex)).IsFail())
                {
                    ErrorPrint(res, "Failed to synchronize window. \n");
                    break;
                }
				...
				// CLabelRemoverDL 객체 생성 // Create CLabelRemoverDL object
				CLabelRemoverDL LabelRemoverDL = new CLabelRemoverDL();
				
				// Source 이미지 설정 // Set source image
				LabelRemoverDL.SetSourceImage(ref fliSourceImage);
				// Destination 이미지 설정 // Set destination image
				LabelRemoverDL.SetDestinationImage(ref fliResultImage);

				// 파라미터 값 설정 // Set parameter value
				LabelRemoverDL.EnablePathMode(false);
                ...
                // 두 개의 이미지 뷰 윈도우의 페이지 인덱스를 동기화 한다 // Synchronize the page index of the two image view windows
                if ((res = viewImageSource.SynchronizePageIndex(ref viewImagesResultRegex)).IsFail())
                {
                    ErrorPrint(res, "Failed to synchronize page index. \n");
                    break;
                }
                ...
                viewImagesResultRegex.ZoomFit();
				viewImageSource.RedrawWindow();
				viewImageResult.RedrawWindow();
                viewImagesResultRegex.RedrawWindow();

				// 이미지 뷰가 종료될 때 까지 기다림 // Wait for the image view to close
                while (viewImageSource.IsAvailable() && viewImageResult.IsAvailable() && viewImagesResultRegex.IsAvailable())
                    Thread.Sleep(1);
