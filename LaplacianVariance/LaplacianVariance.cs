	...
	class LaplacianVariance
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				// Laplacian Variance 객체 생성 // Create Laplacian Variance object
				CLaplacianVariance laplacianVariance = new CLaplacianVariance();

				// Source 이미지 1 설정 // Set the source1 image
				laplacianVariance.SetSourceImage(ref fliSrcImage1);

				//Threshold 설정 //Set Threshold
				laplacianVariance.SetThreshold(5.0);

				// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
				if((res = laplacianVariance.Execute()).IsFail())
				{
					ErrorPrint(res, "Failed to execute Laplacian Variance.");
					break;
				}
				...
				laplacianVariance.SetSourceImage(ref fliSrcImage2);
				...
				laplacianVariance.SetSourceImage(ref fliSrcImage3);
				...
				// 결과 점수 획득 // Get Result Score
				double f64Score3 = laplacianVariance.GetResultScore();
