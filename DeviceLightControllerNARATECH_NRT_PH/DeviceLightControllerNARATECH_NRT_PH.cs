...
{
	class DeviceLightControllerNARATECH_NRT_PH
	{
		...
		static void Main(string[] args)
		{
			...
			CDeviceLightControllerNARATECH_NRT_PH lightControllerNARATECH_NRT_PH = new CDeviceLightControllerNARATECH_NRT_PH();
			...
			do
			{
				...
				if(int.TryParse(Console.ReadLine(), out var portNumber))
				{
					...
					lightControllerNARATECH_NRT_PH.SetComPortNumber(portNumber);
				}
				...
				if(int.TryParse(Console.ReadLine(), out var baudRate))
				{
					...
					lightControllerNARATECH_NRT_PH.SetBaudRate(baudRate);
				}

				if(lightControllerNARATECH_NRT_PH.Initialize().IsFail())
				{
					...
					if(int.TryParse(Console.ReadLine(), out var channel))
					{
						if(operationMode == 1)
						{
							...
							if(int.TryParse(Console.ReadLine(), out var onOff))
							{
								...
								lightControllerNARATECH_NRT_PH.SetChannelState(channel, onOff == 1);
							}
						}
						else if(operationMode == 2)
						{
							...
							if(int.TryParse(Console.ReadLine(), out var lightValue))
							{
								...
								lightControllerNARATECH_NRT_PH.SetLightValue(channel, (byte)lightValue);
							}
						}
					}
			...
			if(lightControllerNARATECH_NRT_PH.Terminate().IsFail())
			{
