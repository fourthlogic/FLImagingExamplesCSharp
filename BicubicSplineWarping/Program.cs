...
namespace BicubicSplineWarping
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				if ((eResult = arrFliImage[0].Load("../../ExampleImages/BicubicSplineWarping/chess.flif")).IsFail())
                {
                ...
                BicubicSplineWarping.SetSourceImage(ref arrFliImage[0]);
                // Destination 이미지 설정 // Set the destination image
                BicubicSplineWarping.SetDestinationImage(ref arrFliImage[1]);
                // Interpolation Method 설정 // Set the interpolation method
                BicubicSplineWarping.SetInterpolationMethod(EInterpolationMethod.Bilinear);
                // Extension 설정
                ...
                BicubicSplineWarping.SetCalibrationPointArray(flpaSource, flpaTarget);
                ...
                {
                    ErrorPrint(eResult, "Failed to calibrate BicubicSplineWarping.");
                ...
                {
                    ErrorPrint(eResult, "Failed to execute BicubicSplineWarping.");
                ...
                // Source 이미지 설정 // Set the source image
                BicubicSplineWarping.SetSourceImage(ref arrFliImage[2]);
                // Destination 이미지 설정 // Set the destination image
                BicubicSplineWarping.SetDestinationImage(ref arrFliImage[3]);
                // Interpolation Method 설정 // Set the interpolation method
                BicubicSplineWarping.SetInterpolationMethod(EInterpolationMethod.Bilinear);
                // Calibration Src, Target Points 바꿔서 셋팅 // Set Calibration Src, Target Points by changing
                BicubicSplineWarping.SetCalibrationPointArray(flpaTarget, flpaSource);
				// Extension 설정
				BicubicSplineWarping.SetExtension(2);
