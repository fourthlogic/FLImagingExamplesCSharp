...
{
	class DeviceLightControllerLVS_PN_40
	{
		...
		static void Main(string[] args)
		{
			...
			// 조명 컨트롤러 LVS_PN_40 선언 // Declare the LVS_PN_40 Light Controller
			CDeviceLightControllerLVS_PN_40 lightControllerLVS_PN40 = new CDeviceLightControllerLVS_PN_40();
			...
			do
			{
				...
				if(int.TryParse(Console.ReadLine(), out var portNumber))
				{
					...
					lightControllerLVS_PN40.SetComPortNumber(portNumber);
				}
				...
				if(int.TryParse(Console.ReadLine(), out var baudRate))
				{
					...
					lightControllerLVS_PN40.SetBaudRate(baudRate);
				}

				if(lightControllerLVS_PN40.Initialize().IsFail())
				{
					...
					if(int.TryParse(Console.ReadLine(), out var channel))
					{
						if(operationMode == 1)
						{
							...
							if(int.TryParse(Console.ReadLine(), out var onOff))
							{
								...
								lightControllerLVS_PN40.SetChannelState(channel, onOff == 1);
							}
						}
						else if(operationMode == 2)
						{
							...
							if(int.TryParse(Console.ReadLine(), out var lightValue))
							{
								...
								lightControllerLVS_PN40.SetLightValue(channel, (byte)lightValue);
							}
						}
					}
			...
			if(lightControllerLVS_PN40.Terminate().IsFail())
			{
