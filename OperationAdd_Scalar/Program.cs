	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				if((eResult = (arrLayer[1].DrawTextCanvas(tpPosition, "Destination1 Image(Add 30, 0, 0)", EColor.YELLOW, EColor.BLACK, 30))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = (arrLayer[2].DrawTextCanvas(tpPosition, "Destination2 Image(Add 0, 30, 0)", EColor.YELLOW, EColor.BLACK, 30))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}
