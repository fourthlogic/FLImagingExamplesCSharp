	...
	class PageReverse
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				CPageReverse pageReverse = new CPageReverse();

				// 처리할 이미지 설정 // Set the image to process
				pageReverse.SetSourceImage(ref fliSrcAfterImage);
				...
				if((res = layerBeforeSrc.DrawTextCanvas(flp, ("Source Image Before Reverse"), EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}

				if((res = layerAfterSrc.DrawTextCanvas(flp, ("Source Image After Reverse"), EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}
