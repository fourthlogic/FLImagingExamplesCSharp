    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            do
            {
				...
				if ((eResult =arrFliImage[0].Load("../../ExampleImages/OperationlOG/gradation_Horizontal.flif")).IsFail())
                {
                ...
                if((eResult =arrViewImage[3].Create(100, 512, 612, 1024)).IsFail())
                {
                    ErrorPrint(eResult,"Failed to create the image view.\n");
                    break;
                }
                ...
                CMultiVar<double> mvScalr = new CMultiVar<double>(2);
                CMultiVar<double> mvScalr2 = new CMultiVar<double>(10);
                CMultiVar<double> mvScalr3 = new CMultiVar<double>(1.5);

                // Operation log 객체 생성 // Create Operation log object
                COperationLog log = new COperationLog();
                // Source 이미지 설정 // Set the source image
                log.SetSourceImage(ref arrFliImage[0]);
                // Destination 이미지 설정 // Set the destination image
                log.SetDestinationImage(ref arrFliImage[1]);
                // Image Operation 모드로 설정 // Set operation mode to image
                log.SetOperationSource(EOperationSource.Scalar);
                // log 값 설정 // Set log value
                log.SetScalarValue(mvScalr);

                // 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
                if((eResult =  log.Execute()).IsFail())
                {
                ...
                // Destination 이미지 설정 // Set the destination image
                log.SetDestinationImage(ref arrFliImage[2]);
                // Image Operation 모드로 설정 // Set operation mode to image
                log.SetOperationSource(EOperationSource.Scalar);
                // log 값 설정 // Set log value
                log.SetScalarValue(mvScalr2);
                ...
                // Destination 이미지 설정 // Set the destination image
                log.SetDestinationImage(ref arrFliImage[3]);
                // Image Operation 모드로 설정 // Set operation mode to image
                log.SetOperationSource(EOperationSource.Scalar);
                // log 값 설정 // Set log value
                log.SetScalarValue(mvScalr3);
                ...
                // 색상 파라미터를 EGUIViewImageLayerTransparencyColor 으로 넣어주게되면 배경색으로 처리함으로 불투명도를 0으로 한것과 같은 효과가 있다. // ifthe color parameter is added as EGUIViewImageLayerTransparencyColor, it has the same effect as setting the opacity to 0 by processing it as a background color.
                ...
                if((eResult =arrLayer[1].DrawTextCanvas(tpPosition, "Destination1 Image(Log 2)", EColor.YELLOW, EColor.BLACK, 30)).IsFail())
                {
                    ErrorPrint(eResult,"Failed to draw text\n");
                    break;
                }

                if((eResult =arrLayer[2].DrawTextCanvas(tpPosition, "Destination2 Image(Log 10)", EColor.YELLOW, EColor.BLACK, 30)).IsFail())
                {
                    ErrorPrint(eResult,"Failed to draw text\n");
                    break;
                }

                if((eResult =arrLayer[3].DrawTextCanvas(tpPosition, "Destination3 Image(Log 1.5)", EColor.YELLOW, EColor.BLACK, 30)).IsFail())
                {
                    ErrorPrint(eResult,"Failed to draw text\n");
                    break;
                }
                ...
                // 이미지 뷰가 종료될 때 까지 기다림 // Wait forthe image view to close
