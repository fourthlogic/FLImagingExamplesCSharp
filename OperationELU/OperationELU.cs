	...
	class OperationELU
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				COperationELU operationELU = new COperationELU();

				if((res = operationELU.SetSourceImage(ref fliSrcImage)).IsFail())
					break;
				if((res = operationELU.SetDestinationImage(ref fliDstImage0)).IsFail())
					break;
				if((res = operationELU.SetOperationMode(COperationELU.EOperationMode.Forward)).IsFail())
					break;
				if((res = operationELU.SetAlpha(1.0)).IsFail())
					break;

				// 알고리즘 수행 // Execute the algorithm
				if((res = operationELU.Execute()).IsFail())
				{
					ErrorPrint(res, "Failed to execute the algorithm.");
					break;
				}

				if((res = operationELU.SetDestinationImage(ref fliDstImage1)).IsFail())
					break;
				if((res = operationELU.SetOperationMode(COperationELU.EOperationMode.Backward)).IsFail())
					break;
