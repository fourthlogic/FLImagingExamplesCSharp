	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				if((eResult = arrFliImage[(int)EType.Source].Load("../../ExampleImages/OperationGreatestCommonDivisor/Sunset.flif")).IsFail())
				{
				...
				if((eResult = (arrLayer[(int)EType.Destination1].DrawTextCanvas(flpZero, "Destination1 Image(GCD 50)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}

				if((eResult = (arrLayer[(int)EType.Destination2].DrawTextCanvas(flpZero, "Destination2 Image(GCD 200)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(eResult, "Failed to draw text.\n");
					break;
				}
