	...
	class RadonTransform
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				CResult res;
				// 이미지 로드 // Load image
				if((res = (arrFliImage[0].Load("../../ExampleImages/RadonTransform/Shepp_logan.flif"))).IsFail())
				{
				...
				// RadonTransform 객체 생성 // Create RadonTransform object
				CRadonTransform radonTransform = new CRadonTransform();
				// Source 이미지 설정 // Set source image
				radonTransform.SetSourceImage(ref arrFliImage[0]);
				// Destination 이미지 설정 // Set destination image 
				radonTransform.SetDestinationImage(ref arrFliImage[1]);
				// 시작 각도 설정 // Set Start Angle
				...
				if((res = (radonTransform.Execute())).IsFail())
				{
					ErrorPrint(res, "Failed to execute Radon Transform.");
					break;
				}
