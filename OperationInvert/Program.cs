	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				CResult eResult;
				// 이미지 로드 // Load image
                if ((eResult = (arrFliImage[0].Load("../../ExampleImages/OperationInvert/board.flif"))).IsFail())
				{
                ...
                // Operation OperationInvert 객체 생성 // Create Invert object
                COperationInvert Invert = new COperationInvert();
				// Source 이미지 설정 // Set source image
                Invert.SetSourceImage(ref arrFliImage[0]);
				// Destination 이미지 설정 // Set destination image 
                Invert.SetDestinationImage(ref arrFliImage[1]);
