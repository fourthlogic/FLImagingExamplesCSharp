	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
                ...
                if ((result = arrFliImage[(int)EType.Source].Load("../../ExampleImages/Illuminate/Basement.flif")).IsFail())
				{
				...
				for(int i = 0; i < (int)EType.ETypeCount; ++i)
				{
					...
					if((result = arrViewImage[i].Create(i * 512 + 100, 0, i * 512 + 100 + 512, 512)).IsFail())
					{
				...
				CIlluminateFilter illuminateFilter = new CIlluminateFilter();
				// Source 이미지 설정 // Set source image
				illuminateFilter.SetSourceImage(ref arrFliImage[(int)EType.Source]);
				...
				{
					ErrorPrint(result, "Failed to execute operation Illuminate Filter.");
					break;
				}
				...
				if((result = arrLayer[(int)EType.Destination1].DrawTextCanvas(flpZero, "IlluminateFilter Size : 41x41 Factor: 0.55", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(result, "Failed to draw text\n");
					break;
				}

                if ((result = arrLayer[(int)EType.Destination2].DrawTextCanvas(flpZero, "IlluminateFilter Size : 151x151 Factor: 0.8", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(result, "Failed to draw text\n");
					break;
				}
