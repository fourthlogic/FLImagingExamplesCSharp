	...
	class LaserTriangulation3D_Profile
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				laserTriangulation3D.SetSourceImage(ref fliSrcImage);
				// Destination Height Map 이미지 설정 // Set the destination height map image
				laserTriangulation3D.SetDestinationHeightMapImage(ref fliDstImage);
				// Destination 3D Object 설정 // Set the Destination 3D Object 
				laserTriangulation3D.SetDestinationObject(ref fl3DOHM);
				// Source 이미지 타입 설정 // Set the type of the source image
				laserTriangulation3D.SetSourceType(CLaserTriangulation3D.ESourceType.Profile);
				...
				if(view3DDst.PushObject(laserTriangulation3D.GetDestinationObject()).IsFail())
				{
					ErrorPrint(res, "Failed to set image object on the image view.\n");
					break;
				}
