	...
	class WaveletFocusMeasurement
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				if((res = fliSrcImage3.Load("../../ExampleImages/FocusMeasurement/Focus3.flif")).IsFail())
				{
					ErrorPrint(res, "Failed to load the image file.\n");
					break;
				}
				...
				if((res = viewImageSrc3.Create(1424, 0, 1936, 612)).IsFail())
				{
					ErrorPrint(res, "Failed to create the image view.\n");
					break;
				}
				...
				if((res = viewImageSrc3.SetImagePtr(ref fliSrcImage3)).IsFail())
				{
					ErrorPrint(res, "Failed to set image object on the image view.\n");
					break;
				}
				...
				if((res = viewImageSrc1.SynchronizePointOfView(ref viewImageSrc3)).IsFail())
				{
					ErrorPrint(res, "Failed to synchronize view\n");
					break;
				}
				...
				if((res = viewImageSrc1.SynchronizeWindow(ref viewImageSrc3)).IsFail())
				{
					ErrorPrint(res, "Failed to synchronize window.\n");
					break;
				}

				// Wavelet Focus Measurement 객체 생성 // Create Wavelet Focus Measurement object
				CWaveletFocusMeasurement waveletFocusMeasurement = new CWaveletFocusMeasurement();

				// 기저 함수 설정 // Set Basis Function
				waveletFocusMeasurement.SetBasisFunction(CWaveletFocusMeasurement.EBasisFunction.Haar);

				// 분해 단계 설정 // Set Decomposition Level
				waveletFocusMeasurement.SetDecompositionLevel(1);

				// Source 이미지 1 설정 // Set the source1 image
				waveletFocusMeasurement.SetSourceImage(ref fliSrcImage1);

				// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
				if((res = waveletFocusMeasurement.Execute()).IsFail())
				{
					ErrorPrint(res, "Failed to execute Wavelet Focus Measurement.");
					break;
				}

				// 결과 점수 획득 // Get Result Score
				double f64Score1 = waveletFocusMeasurement.GetResultScore();

				// Source 이미지 2 설정 // Set the source2 image
				waveletFocusMeasurement.SetSourceImage(ref fliSrcImage2);
				...
				// 결과 점수 획득 // Get Result Score
				double f64Score2 = waveletFocusMeasurement.GetResultScore();

				// Source 이미지 3 설정 // Set the source3 image
				waveletFocusMeasurement.SetSourceImage(ref fliSrcImage3);
				...
				// 결과 점수 획득 // Get Result Score
				double f64Score3 = waveletFocusMeasurement.GetResultScore();
				...
				CGUIViewImageLayer layerSrc3 = viewImageSrc3.GetLayer(0);
				...
				if((res = layerSrc3.DrawTextCanvas(new CFLPoint<double>(0, 0), "Score : " + f64Score3, EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}
