	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				CSigmaFilter sigmaFilter = new CSigmaFilter();
				// Source 이미지 설정 // Set source image
				sigmaFilter.SetSourceImage(ref arrFliImage[(int)EType.Source]);
                ...
                if ((result = arrLayer[(int)EType.Destination1].DrawTextCanvas(flpZero, "SigmaFilter Size : 31x31 Sigma: 3", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(result, "Failed to draw text\n");
					break;
				}

                if ((result = arrLayer[(int)EType.Destination2].DrawTextCanvas(flpZero, "SigmaFilter Size : 101x101 Sigma: 3", EColor.YELLOW, EColor.BLACK, 20)).IsFail())
				{
					ErrorPrint(result, "Failed to draw text\n");
					break;
				}
