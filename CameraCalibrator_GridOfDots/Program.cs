    ...
    class Program
	{
        ...
        static void Main(string[] args)
        {
			...
			do
			{
                ...
                if ((eResult = fliLearnImage.Load("../../ExampleImages/CameraCalibrator/GridOfDots/GridOfDots.flif")).IsFail())
                {
                ...
                if ((eResult = fliSourceImage.Load("../../ExampleImages/CameraCalibrator/GridOfDots/GridOfDots (1).flif")).IsFail())
                {
                    ErrorPrint(eResult, "Failed to load the image file.");
                    break;
                }
                ...
                for (int i = 0; i < 3; ++i)
                {
                    ...
                    for (int i64Row = 0; i64Row < i64GridRow; ++i64Row)
                    {
                        ...
                        for (int i64Col = 0; i64Col < i64GridCol; ++i64Col)
                        {
                            ...
                            if ((eResult = layerLearn.DrawTextImage(sArrGridDisplay[i].sGridData.arrGridData[i64Row][i64Col], String.Format("{0}", i32VertexNumber++), crTextColor, EColor.BLACK, (int)(f64Pitch / 3), true, f64AngleIner)).IsFail())
                            {
                                ErrorPrint(eResult, "Failed to draw figure.\n");
                                break;
                            }
