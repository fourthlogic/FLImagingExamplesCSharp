...
{
	class CameraCalibrator_GridOfDots
	{
		...
		static bool Calibration(CCameraCalibrator cameraCalibrator, CFLImage fliLearnImage)
		{
			...
			do
			{
				...
				cameraCalibrator.SetGridType(CCameraCalibrator.EGridType.GridOfDots);
				// 결과에 대한 학습률을 설정합니다.
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				for(int i = 0; i < 3; ++i)
				{
					...
					for(int i64Row = 0; i64Row < i64GridRow; ++i64Row)
					{
						CFLPoint<double> pFlpGridPoint1 = (CFLPoint<double>)(((CFLFigureArray)sArrGridDisplay[i].sGridData.flfaGridData.GetAt(i64Row)).GetAt(0));
						CFLPoint<double> pFlpGridPoint2 = (CFLPoint<double>)(((CFLFigureArray)sArrGridDisplay[i].sGridData.flfaGridData.GetAt(i64Row)).GetAt(1));
						...
						for(int i64Col = 0; i64Col < i64GridCol; ++i64Col)
						{
							...
							if(i64Row == 0)
							{
								...
								{
									pFlpGridPoint1 = (CFLPoint<double>)(((CFLFigureArray)sArrGridDisplay[i].sGridData.flfaGridData.GetAt(i64Row + 1)).GetAt(i64Col));
									pFlpGridPoint2 = (CFLPoint<double>)(((CFLFigureArray)sArrGridDisplay[i].sGridData.flfaGridData.GetAt(i64Row + 1)).GetAt(i64Col + 1));
								}
								else
								{
									pFlpGridPoint1 = (CFLPoint<double>)(((CFLFigureArray)sArrGridDisplay[i].sGridData.flfaGridData.GetAt(i64Row + 1)).GetAt(i64Col));
									pFlpGridPoint2 = (CFLPoint<double>)(((CFLFigureArray)sArrGridDisplay[i].sGridData.flfaGridData.GetAt(i64Row + 2)).GetAt(0));
								}
							...
							{
								pFlpGridPoint1 = (CFLPoint<double>)(((CFLFigureArray)sArrGridDisplay[i].sGridData.flfaGridData.GetAt(i64Row)).GetAt(i64Col));
								pFlpGridPoint2 = (CFLPoint<double>)(((CFLFigureArray)sArrGridDisplay[i].sGridData.flfaGridData.GetAt(i64Row - 1)).GetAt(i64Col));
							...
							if((res = layerLearn.DrawTextImage((CFLPoint<double>)(((CFLFigureArray)sArrGridDisplay[i].sGridData.flfaGridData.GetAt(i64Row)).GetAt(i64Col)), String.Format("{0}", i32VertexNumber++), crTextColor, EColor.BLACK, (int)(f64Pitch / 3), true, f64AngleIner)).IsFail())
							{
								ErrorPrint(res, "Failed to draw figure.\n");
								break;
							}
							...
							if(i64Col > 0)
							{
								CFLPoint<double> flpAngle0 = new CFLPoint<double>((CFLPoint<double>)(((CFLFigureArray)sArrGridDisplay[i].sGridData.flfaGridData.GetAt(i64Row)).GetAt(i64Col - 1)));
								CFLPoint<double> flpAngle1 = new CFLPoint<double>((CFLPoint<double>)(((CFLFigureArray)sArrGridDisplay[i].sGridData.flfaGridData.GetAt(i64Row)).GetAt(i64Col)));
