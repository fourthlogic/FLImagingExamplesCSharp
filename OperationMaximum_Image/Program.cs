	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				CResult eResult;
				// 이미지 로드 // Load image
				if((eResult = (arrFliImage[0].Load("../../ExampleImages/OperationMaximum/palmtree.flif"))).IsFail())
				{
				...
				// 이미지 로드 // Load image
				if((eResult = (arrFliImage[1].Load("../../ExampleImages/OperationMaximum/Flower.flif"))).IsFail())
				{
					ErrorPrint(eResult, "Failed to load the image file.\n");
					break;
				}
				...
				if((eResult = (eResult = maximum.Execute())).IsFail())
				{
