	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				layer[1].DrawTextCanvas(new CFLPoint<int>(0, 0), "Open Result1", EColor.YELLOW, EColor.BLACK, 30);
				layer[2].DrawTextCanvas(new CFLPoint<int>(0, 0), "Open Result2", EColor.YELLOW, EColor.BLACK, 30);
				layer[3].DrawTextCanvas(new CFLPoint<int>(0, 0), "Open Result2", EColor.YELLOW, EColor.BLACK, 30);
				...
				if((res = flrgSourceFig.Open(i64KernelSize, i64KernelSize, ref flfaResult1)).IsFail())
				{
				...
				if((res = flrgSourceFig.Open(i64KernelSize, i64KernelSize, ref flfaResult2, EKernelShape.Circle)).IsFail())
				{
					ErrorPrint(res, "Failed to calculate.\n");
					break;
				}
				...
				if((res = flrgSourceFig.Open(fleKernel, ref flfaResult3)).IsFail())
				{
					ErrorPrint(res, "Failed to calculate.\n");
					break;
				}
