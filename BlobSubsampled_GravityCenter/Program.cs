	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				if((eResult = sBlob.Filter(CBlob.EFilterItem.BoundaryRectWidth, 20, ELogicalCondition.Less)).IsFail())
				{
					ErrorPrint(eResult, "BlobSubsampled filtering algorithm error occurs.");
					break;
				}

				// BoundaryRect의 20보다 작은 높이를 가진 객체들을 제거
				if((eResult = sBlob.Filter(CBlob.EFilterItem.BoundaryRectHeight, 20, ELogicalCondition.Less)).IsFail())
				{
				...
				if((eResult = sBlob.GetResultGravityCenters(out flfGravityCenter)).IsFail())
				{
				...
				if((eResult = layer.DrawFigureImage(flfaContours, EColor.BLUE, 1, EColor.BLUE, EGUIViewImagePenStyle.Solid, 1.0f, .25f)).IsFail())
				{
