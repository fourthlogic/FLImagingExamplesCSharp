...
{
	class OperationBitShift_Scalar
	{
		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				if((res = arrFliImage[(int)EType.Source].Load("../../ExampleImages/OperationShift/Flower.flif")).IsFail())
				{
				...
				if((res = (arrLayer[(int)EType.Destination1].DrawTextCanvas(flpZero, "Destination1 Image(left 1)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}

				if((res = (arrLayer[(int)EType.Destination2].DrawTextCanvas(flpZero, "Destination2 Image(right 2)", EColor.YELLOW, EColor.BLACK, 20))).IsFail())
				{
					ErrorPrint(res, "Failed to draw text.\n");
					break;
				}
